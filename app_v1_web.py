...
<style>
    ...
    /* Expander header word wrap enforcement */
...
DEFAULT_RAW_ZIPS_DIR = "/Personal/전광용/DM Agent/raw_zips"
...
def show_remote_folder_dialog(ftp_host, ftp_port, ftp_user, ftp_pass):
    ...
    with col_confirm:
        if st.button("✅ 이 폴더를 작업 경로로 선택", type="primary", use_container_width=True, key="dlg_web_btn_confirm_dir"):
            ...
            st.session_state['input_active_remote_dir'] = cur_nav
            st.session_state['input_active_remote_dir_web'] = cur_nav
...
def extract_scenario_summary_tables(excel_source):
    """
    마스터 엑셀 각 단말 시트의 [1. 전체 종합 요약] 첫 번째 데이터 행을 그대로 1:1 수집하여 표출.
    - DL 시트들(M1_DL, M2_DL 등)의 1단 종합 요약 첫 행 -> DL 테이블
    - UL 시트들(M1_UL 등)의 1단 종합 요약 첫 행 -> UL 테이블
    - Ping 시트들(M1_Ping 등)의 1단 종합 요약 첫 행 -> Ping 테이블
    - Voice 시트들(M3_Voice, M4_Voice 등)의 1단 종합 요약 첫 행 -> Voice 테이블
    (어떠한 재연산도 없이 엑셀 1단 대표 요약 행을 100% 그대로 반영)
    """
    ...
    for s_name in sheets:
        ...
        if any(k in s_lower for k in ['l3_', '감사', '01_통합', 'mobility_meas']):
            continue

        if any(k in s_lower for k in ['voice', 'volte', 'vonr']):
            cat = 'Voice'
        elif 'ping' in s_lower:
            cat = 'Ping'
        elif 'ul' in s_lower and 'pul' not in s_lower:
            cat = 'UL'
        else:
            cat = 'DL'
        ...
        try:
            ...
            block1_idx = None
            for r_i in range(min(15, len(df_raw))):
                cell_0 = str(df_raw.iloc[r_i, 0])
                if '1.' in cell_0 and '전체' in cell_0 and '요약' in cell_0:
                    block1_idx = r_i
                    break

            if block1_idx is not None:
                hdr_idx = block1_idx + 1
                data_idx = block1_idx + 2
                if data_idx < len(df_raw):
                    headers = [str(c).strip() if pd.notna(c) else f"Col_{c_i}" for c_i, c in enumerate(df_raw.iloc[hdr_idx].tolist())]
                    vals = df_raw.iloc[data_idx, :len(headers)].tolist()
                    row_dict = {"시트 (단말)": s_name}
                    for h, v in zip(headers, vals):
                        if h and h != 'None':
                            row_dict[h] = v
                    scenario_rows[cat].append(row_dict)
            else:
                df_direct = xl.parse(s_name)
                if not df_direct.empty:
                    first_row = df_direct.iloc[0].to_dict()
                    first_row["시트 (단말)"] = s_name
                    scenario_rows[cat].append(first_row)
        except Exception:
            continue

    result_tables = {}
    for cat, rows in scenario_rows.items():
        if rows:
            result_tables[cat] = pd.DataFrame(rows)
...
def extract_all_per_sec_sheets(excel_source) -> Dict[str, pd.DataFrame]:
    """
    마스터 엑셀의 각 단말 시트(M1_DL 등)에서 [3. 초(sec) 단위 결과] 테이블을 정확히 추출.
    """
    sheets = {}
    try:
        if isinstance(excel_source, bytes):
        ...
        for s in xl.sheet_names:
            ...
            try:
                ...
                block3_idx = None
                for r_i in range(len(df_raw)):
                    cell_0 = str(df_raw.iloc[r_i, 0])
                    if '3.' in cell_0 and ('초' in cell_0 or 'sec' in cell_0) and '단위' in cell_0:
                        block3_idx = r_i
                        break

                if block3_idx is not None and block3_idx + 1 < len(df_raw):
                    hdr_idx = block3_idx + 1
                    raw_headers = [str(c).strip() if pd.notna(c) else f"Col_{c_i}" for c_i, c in enumerate(df_raw.iloc[hdr_idx].tolist())]
                    df_sec = df_raw.iloc[hdr_idx + 1:].copy()
                    df_sec = df_sec.dropna(how='all')
                    df_sec.columns = raw_headers[:len(df_sec.columns)]
                    ts_col = next((c for c in df_sec.columns if 'time' in str(c).lower()), None)
                    if ts_col:
                        df_sec['TIME_STAMP'] = df_sec[ts_col]
                    sheets[s] = df_sec.reset_index(drop=True)
                else:
                    if 'per_sec' in s_lower or 'timeline' in s_lower:
                        sheets[s] = xl.parse(s)
            except Exception:
                continue
    except Exception:
...
default_cache_dir = secrets_ftp.get('remote_cache_dir') or saved_cfg.get('remote_cache_dir', '/Personal/전광용/DM Agent/sessions')
if not default_cache_dir or '260728_Analyzer Tool' in default_cache_dir:
    default_cache_dir = '/Personal/전광용/DM Agent/sessions'
...
with st.sidebar:
    ...
    if cached_list:
        search_kw = st.text_input(
            "🔍 세션 검색:",
            value="",
            placeholder="노선명, 날짜 (예: 이천, 0907, 상무)",
            key="txt_session_search_kw_web",
            help="입력한 키워드가 포함된 세션만 목록에 즉시 필터링됩니다."
        ).strip()

        if search_kw:
            filtered_sessions = [s for s in cached_list if search_kw.lower() in str(s).lower()]
        else:
            filtered_sessions = cached_list

        if filtered_sessions:
            selected_cache = st.selectbox(
                f"시연/분석 세션 선택 ({len(filtered_sessions)}개):",
                options=filtered_sessions,
                index=0,
                key="sb_selected_cache_web",
            ...
            st.caption(f"⚠️ '{search_kw}' 검색 결과가 없습니다.")
            if st.button("🔄 검색어 초기화", key="btn_clear_kw_web", use_container_width=True):
                st.session_state['txt_session_search_kw_web'] = ""
                st.rerun()
    else:
        ...
        for sk in s_keys:
            ...
            with col_exp:
                with st.expander(expander_title, expanded=is_cur):
                    ...
                    port_reps = cur_dl.get('port_txt_reports', {})
                    if port_reps and len(port_reps) > 1:
                        st.markdown("<div style='font-size: 11px; color: #94a3b8; margin-top: 4px; margin-bottom: 2px;'>단말 포트별 개별 리포트:</div>", unsafe_allow_html=True)
                        cols_p = st.columns(min(len(port_reps), 4))
                        for c_i, (p_k, p_txt) in enumerate(sorted(port_reps.items())):
                            col_target = cols_p[c_i % len(cols_p)]
                            with col_target:
                                st.download_button(
                                    label=f"📄 {p_k}",
                                    data=p_txt,
                                    file_name=f"Optis_{sk}_{p_k}_Report.txt",
                                    mime="text/plain",
                                    key=f"dl_pt_{sk}_{p_k}",
                                    use_container_width=True
                                )
    ...
    st.markdown("---")
    # Clean connection status badge
    st.markdown(
        """<div style="background-color: #1e293b; border: 1px solid #334155; border-radius: 6px; padding: 6px 10px; margin-bottom: 8px;">
    ...
    # -------------------------------------------------------------------------
    # 1. 📁 원격 작업 폴더 설정 (라이센스 매니저 계정별 경로 직접 변경)
    # -------------------------------------------------------------------------
    st.markdown("##### 📁 원격 작업 폴더 (ZIP 경로)")

    # 경로 직접 입력/수정 지원 (on_change 이벤트 기반으로 역방향 덮어쓰기 원천 차단)
    if 'input_active_remote_dir_web' not in st.session_state or st.session_state['input_active_remote_dir_web'] != active_dir:
        st.session_state['input_active_remote_dir_web'] = active_dir

    def on_remote_dir_change_web():
        typed_dir = st.session_state.get('input_active_remote_dir_web', '').strip().replace('\\', '/')
        if typed_dir and typed_dir != st.session_state.get('active_remote_dir'):
            st.session_state['active_remote_dir'] = typed_dir
            st.session_state['input_active_remote_dir_web'] = typed_dir
            zips = list_remote_zip_files(ftp_host, ftp_port, ftp_user, ftp_pass, typed_dir)
            st.session_state['ftp_zip_list'] = zips
            st.session_state['ftp_zip_list_dir'] = typed_dir

    st.text_input(
        "원격 폴더 경로:",
        key="input_active_remote_dir_web",
        on_change=on_remote_dir_change_web,
        help="라이센스 매니저 계정 또는 ZIP 파일이 위치한 원격 경로를 지정합니다. 엔터를 치면 즉시 적용됩니다."
    )

    col_browse, col_reload = st.columns([0.65, 0.35])
    with col_browse:
        if st.button("🔍 폴더 탐색기", use_container_width=True, key="btn_web_open_folder_dlg", help="원격 폴더 트리를 탐색하여 선택합니다."):
            st.session_state['browse_nav_dir'] = active_dir
            st.session_state['show_explorer_dialog'] = True
            st.rerun()
    with col_reload:
        if st.button("🔄 새로고침", use_container_width=True, key="btn_web_reload_zips", help="현재 폴더의 ZIP 파일 목록을 다시 조회합니다."):
            with st.spinner("ZIP 검색 중..."):
            ...
            try:
                ...
                if results:
                    for r_key, r_val in results.items():
                        ...
                        sample_seed = r_val.get('measurement_date')
                        if not sample_seed:
                            for pk, pdata in r_val.get('port_results', {}).items():
                                if pdata.get('date_str'):
                                    sample_seed = pdata['date_str']
                                    break

                        if not sample_seed and dl_zips:
                            for dz in dl_zips:
                                bn = os.path.basename(dz)
                                if any(yr in bn for yr in ['20', '24', '25', '26']) or re.search(r'(0[1-9]|1[0-2])([0-3]\d)', bn):
                                    sample_seed = bn
                                    break
                        ...
                        saved_dir = cache_mgr.save_local_session(
                            ...
                            meta_dict=meta_dict,
                            port_txt_reports=r_val.get('port_txt_reports', {})
                        )
...
# Scroll to top trigger handling
if st.session_state.get('scroll_to_top'):
    st.session_state['scroll_to_top'] = False
    components.html("""
    <script>
        window.parent.document.querySelectorAll('[data-testid="stAppViewContainer"], section.main, .main').forEach(el => {
