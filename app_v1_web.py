...
def extract_scenario_summary_tables(excel_source):
    ...
    # -------------------------------------------------------------------------
    # 2. Fallback: Parse 3-tier vertical sheets (e.g. 'M1', 'M2', 'M3', 'M4')
    # -------------------------------------------------------------------------
    if not result_tables:
        port_sheets = [s for s in sheets if re.match(r'^M\d+$', s.strip(), re.IGNORECASE) or s.strip().upper() in ['M1', 'M2', 'M3', 'M4']]
        tier2_rows_by_sc = {'DL': [], 'UL': [], 'Ping': [], 'Voice': []}

        for ps in port_sheets:
            try:
                df_raw = xl.parse(ps, header=None)
                if df_raw.empty:
                    continue

                # Locate Block 2: Call 단위 결과
                block2_idx = None
                block3_idx = None
                for r_i in range(len(df_raw)):
                    cell_0 = str(df_raw.iloc[r_i, 0])
                    if '2.' in cell_0 and 'Call' in cell_0 and '단위' in cell_0:
                        block2_idx = r_i
                    elif '3.' in cell_0 and '초' in cell_0 and '단위' in cell_0:
                        block3_idx = r_i
                        break

                if block2_idx is not None:
                    hdr_idx = block2_idx + 1
                    end_idx = block3_idx if block3_idx is not None else len(df_raw)
                    raw_headers = [str(c).strip() if pd.notna(c) else f"Col_{c_i}" for c_i, c in enumerate(df_raw.iloc[hdr_idx].tolist())]

                    data_sub = df_raw.iloc[hdr_idx+1:end_idx].copy()
                    data_sub = data_sub.dropna(how='all')
                    data_sub = data_sub[~data_sub.iloc[:, 0].astype(str).str.contains('■|단위 결과', na=False)]

                    if not data_sub.empty:
                        data_sub.columns = raw_headers[:len(data_sub.columns)]

                        # Detect scenario accurately from headers and content
                        cols_str = ' '.join(raw_headers).lower()
                        vals_str = ' '.join(str(x) for x in data_sub.values.flatten() if pd.notna(x)).lower()

                        if any(k in cols_str for k in ['mos', 'jitter', 'codec', 'rtp']) or any(k in vals_str for k in ['voice', 'volte', 'vonr']):
                            sc_detected = 'Voice'
                        elif 'ping' in cols_str or 'rtt' in cols_str or 'ping' in vals_str:
                            sc_detected = 'Ping'
                        elif 'ul 속도' in cols_str or 'pusch' in cols_str or 'ul' in vals_str:
                            sc_detected = 'UL'
                        else:
                            sc_detected = 'DL'

                        total_calls = len(data_sub)
                        res_col = next((c for c in data_sub.columns if any(k in str(c).lower() for k in ['결과', 'result', 'status', 'state', '상태'])), None)
                        if res_col:
                            success_count = sum(data_sub[res_col].astype(str).str.contains('정상|완료|성공|Success|PASS|Pass', case=False, regex=True))
                            succ_rate = round(success_count / total_calls * 100.0, 1) if total_calls > 0 else 0.0
                            call_stat_str = f"{success_count} / {total_calls} Call ({succ_rate}%)"
                        else:
                            call_stat_str = f"{total_calls} / {total_calls} Call (100.0%)"
                        ...
                        row_dict = {
                            "단말 (포트)": f"{ps.strip()} ({sc_detected})",
                            "호 성공 / 시도": call_stat_str
                        }

                        for col in data_sub.columns:
                            if col == res_col or col in ['Call_No', '호 번호', 'No', 'Index', '시작시간', '종료시간', 'Time', 'Timestamp', '시간 (구간)']:
                                continue
                            num_s = pd.to_numeric(data_sub[col], errors='coerce').dropna()
                            if len(num_s) > 0:
                                if any(k in str(col).lower() for k in ['회수', 'count', '건수', 'num', '횟수']):
                                    row_dict[col] = int(num_s.sum())
                                else:
                                ...
                                non_nulls = data_sub[col].dropna()
                                if len(non_nulls) > 0:
                                    row_dict[col] = str(non_nulls.iloc[0])

                        tier2_rows_by_sc[sc_detected].append(row_dict)
            except Exception:
                continue

        for sc_name, r_list in tier2_rows_by_sc.items():
            if r_list:
                result_tables[sc_name] = pd.DataFrame(r_list)
