...
with st.sidebar:
    ...
    if input_dir and input_dir.strip() != active_dir:
        new_clean_dir = input_dir.strip().replace('\\', '/')
        st.session_state['active_remote_dir'] = new_clean_dir
        with st.spinner("폴더 변경 및 ZIP 목록 갱신 중..."):
            zips = list_remote_zip_files(ftp_host, ftp_port, ftp_user, ftp_pass, new_clean_dir)
            st.session_state['ftp_zip_list'] = zips
            st.session_state['ftp_zip_list_dir'] = new_clean_dir
        st.rerun()
