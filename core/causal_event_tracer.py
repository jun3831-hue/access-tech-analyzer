...
from core.quality_criteria_registry import (
    get_rsrp_evaluation, get_sinr_evaluation,
    is_clean_rf_condition, is_pilot_pollution_condition, is_weak_coverage_condition,
    is_rapid_rsrp_drop
)
...
class CausalEventTracer:
    ...
    def trace_episodes(
        ...
        for ev in all_events:
            ...
            # 3. Direct Causal Escalation: Mobility/RACH trigger -> Subsequent Link Failure within 15.0s (Too Late HO / Target RACH Failure -> Call Drop)
            has_mobility_or_rach_trigger = any(_get_family(e) in ['HO_MOBILITY', 'RACH_ACCESS'] for e in cluster)
            is_subsequent_failure = (curr_fam in ['LINK_FAILURE', 'RACH_ACCESS'])
            cell_matches_failure = bool(curr_cells & cluster_visited_cells) if (curr_cells and cluster_visited_cells) else True
            causal_escalation_merge = (has_mobility_or_rach_trigger and is_subsequent_failure and cell_matches_failure and time_gap <= 15.0)
    ...
    def _synthesize_episode(
        ...
        # Determine Specific 3GPP Signals (Strictly require rep_cnt >= 1 for unhandled HO)
        has_unhandled_ho = any('핸드오버 지시 미발행' in e['name'] or 'A3 방치' in e['name'] or 'A3 UNHANDLED' in str(e.get('diag_code', '')) for e in cluster) and (rep_cnt is not None and rep_cnt >= 1)
        ...
        sinr_val = srv_sinr if srv_sinr is not None else -20.0

        # Extract 3GPP Failure Cause (1st priority: protocol field or event detail code)
        std_cause = next((e.get('cause_code') or e.get('standard_cause') for e in cluster if (e.get('cause_code') or e.get('standard_cause'))), None)
        if not std_cause:
            for e in cluster:
                det = str(e.get('detail', '')) + " " + str(e.get('name', ''))
                if 'RACH Problem' in det or 'randomaccessproblem' in det.lower():
                    std_cause = 'randomAccessProblem'
                    break
                elif 'T304' in det or 'handoverfailure' in det.lower() or 'Handover Failure' in det:
                    std_cause = 'handoverFailure'
                    break
                elif 'Max Retx' in det or 'rlc-maxnumretx' in det.lower():
                    std_cause = 'rlc-MaxNumRetx'
                    break
                elif 'T310' in det or 't310-expiry' in det.lower():
                    std_cause = 't310-Expiry'
                    break

        # Single SSOT rapid RSRP drop guard
        is_drop = is_rapid_rsrp_drop(delta_rsrp, duration_sec)
        drop_fact = f"서빙 전계 급락(ΔRSRP {delta_rsrp:+.1f}dB)" if (is_drop and delta_rsrp is not None) else None

        if is_weak_coverage_condition(rsrp_val):
            rf_fact_clause = f"커버리지 음영(RSRP {rsrp_val:.1f}dBm)"
        elif is_pilot_pollution_condition(rsrp_val, sinr_val):
            rf_fact_clause = f"채널 간섭 우세(SINR {sinr_val:.1f}dB)"
        elif drop_fact:
            rf_fact_clause = f"{drop_fact} 및 채널 품질(SINR {sinr_val:.1f}dB)"
        else:
            rf_fact_clause = f"정상 무선 품질(RSRP {rsrp_val:.1f}dBm, SINR {sinr_val:.1f}dB)"
        ...
        if has_unhandled_ho and has_actual_call_drop:
            ...
            candidates.append((1, t, c))
        elif std_cause == 'handoverFailure' or ('HO Failure' in str(t) if 't' in locals() else False):
            t = f"{rat_prefix}타겟({tgt_label}) 핸드오버 실패 및 RRE 거절 호 단절"
            c = f"서빙 셀({srv_label})에서 타겟 셀({tgt_label})로의 핸드오버 실패(T304 만료 또는 타겟 RACH 실패) 후 RRE 거절로 인한 최종 호 단절 발발"
            candidates.append((1, t, c))
        elif has_rre_rej and has_rlf:
            cause_tag = f" ({std_cause})" if std_cause else ""
            t = f"{rat_prefix}RLF 및 RRE 거절 호 단절{cause_tag}"
            c = f"서빙 셀({srv_label})의 {rf_fact_clause} 환경에서 무선 링크 단절(RLF, 3GPP 원인: {std_cause or '링크 상실'}) 발생 후 단말이 링크 재수립을 요청했으나 대상 기지국 컨텍스트 부재로 ReestablishmentReject 회신 및 최종 호 단절 발발"
            candidates.append((1, t, c))
        elif has_rre_rej:
            ...
            c = f"수신 전계(RSRP {rsrp_val:.1f}dBm, {rf_fact_clause}) 상태에서 기지국 제어 결함 또는 베어러 비정상 해제로 인한 통화 단절"
            candidates.append((1, t, c))
        elif has_rach_problem or std_cause == 'randomAccessProblem':
            t = f"{rat_prefix}상향 RACH 실패 RLF (randomAccessProblem)"
            c = f"수신 전계(RSRP {rsrp_val:.1f}dBm, {rf_fact_clause}) 상태에서 Preamble 최대 전송 횟수 도달 후 기지국 무응답으로 상향 RACH Problem RLF 발발"
            candidates.append((1, t, c))
        ...
        for e in cluster:
            ...
            if 'RACH Problem' in name or 'PRACH' in name:
                ...
                rlf_c = e.get('cause_code') or e.get('standard_cause') or std_cause or '무선 링크 실패'
                story_steps.append(f"[{ts_str}] 무선 링크 단절(RLF, 3GPP 원인: {rlf_c}) 발발")
            elif 'Reestablishment Reject' in name or '재수립 거절' in name:
                reest_c = e.get('cause_code') or e.get('standard_cause') or std_cause or 'otherFailure'
                story_steps.append(f"[{ts_str}] 단말의 RRCConnectionReestablishmentRequest(cause: {reest_c}) 요청에 대해 기지국이 ReestablishmentReject를 회신함")
    ...
    def format_incident_report(self, episodes: List[Dict[str, Any]]) -> str:
        ...
        def _fmt_ts(val):
            ...
            s = str(val).split(' ')[-1]
            return s[:12] if len(s) >= 8 else s

        # Calculate FSI and classify each episode (Strict HIGH, MED, LOW Standard)
        graded_episodes = []
        for ep in episodes:
            title_text = ep.get('title') or ep.get('summary', '장애 구간')
            story_steps = ep.get('story_steps', [])
            steps_text = " ".join(story_steps)
            full_text = f"{title_text} {steps_text}"

            has_high_event = any(ev.get('severity') == 'HIGH' for ev in ep.get('events', []))
            is_high_text = any(k in full_text for k in ['SCG Failure', 'SCG Fail', 'Radio Link Failure', 'RLF', 'Reestablishment Reject', '호 절단', 'Call Drop', 'e-RAB Drop', '2회 왕복', '3회 왕복', '연속 핑퐁', '중복 PCI', 'RTP Drop'])
            ...
            is_med_text = any(k in full_text for k in ['핸드오버 지시 지연', '핸드오버 지시 미발행', '핑퐁', '미수행', '실행 지연', 'TAU', '묵음'])
            ...
            t_dur = max(0.0, float(ep.get('duration_sec', ep.get('dur_sec', 0.0))))
            n_ev = ep.get('total_events', len(ep.get('events', [])))
            ...
            ep_copy['fsi'] = fsi
            ep_copy['grade'] = grade
            ep_copy['title'] = title_text
            ep_copy['duration_sec'] = t_dur
            ep_copy['story_steps'] = story_steps
            graded_episodes.append(ep_copy)

        high_and_med = [e for e in graded_episodes if e['grade'] in ['HIGH', 'MED']]
        # Sort: HIGH first (grade == 'HIGH' -> 0, MED -> 1), then chronological
        high_and_med.sort(key=lambda x: (0 if x['grade'] == 'HIGH' else 1, str(x.get('t_start', x.get('time_stamp', '')))))
        ...
        for ep in high_and_med:
            srv_rat = ep.get('srv_rat') or ep.get('rat') or 'LTE'
            ts_start_str = _fmt_ts(ep.get('t_start', ep.get('time_stamp')))
            ts_end_str = _fmt_ts(ep.get('t_end', ep.get('time_stamp')))
            lines.append(f"┌──────────────────────────────────────────────────────────────────────────────────────────────────┐")
            ...
            srv_pci_val = ep.get('srv_pci', ep.get('pci'))
            ...
            cause_text = ep.get('cause_conclusion') or ep.get('root_cause') or '상세 원인 미상'
        ...
        if low_eps:
            ...
            for m in low_eps:
                m_start = _fmt_ts(m.get('t_start', m.get('time_stamp')))
                m_title = m.get('title', m.get('summary', ''))
                lines.append(f"  • [{m_start}] {m.get('duration_sec', 0.0):.1f}초간 {m_title}")
            lines.append(sub_div)
