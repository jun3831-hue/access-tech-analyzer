...
from core.quality_criteria_registry import is_rapid_rsrp_drop
...
class MobilityDetector:
    ...
    def extract_mobility_transitions(self, df_mob: pd.DataFrame, rat: str = 'NR') -> List[Dict[str, Any]]:
        if df_mob is None or df_mob.empty:
            return []

        df_work = df_mob.copy()
        if 'RAT' not in df_work.columns:
            has_nr = ('NR_SS_RSRP' in df_work.columns and not df_work['NR_SS_RSRP'].dropna().empty) or \
                     ('NR_Serving_PCI' in df_work.columns and (pd.to_numeric(df_work['NR_Serving_PCI'], errors='coerce') > 0).any())
            df_work['RAT'] = 'NR' if has_nr else 'LTE'

        df_rat = df_work[df_work['RAT'] == rat].copy()
        if df_rat.empty:
            if rat == 'LTE' and not (df_work['RAT'] == 'NR').all():
                df_rat = df_work.copy()
            else:
                return []

        pci_col = 'NR_Serving_PCI' if rat == 'NR' else 'LTE_Serving_PCI'
        if pci_col not in df_rat.columns:
            for cand in ['Serving_PCI', 'pci', 'PCI', '[Call & LTE KPI PCell Serving PCI]', '[Call & 5G KPI PCell RF Serving PCI]']:
                if cand in df_rat.columns:
                    pci_col = cand
                    break
            else:
                pci_col = None

        if 'HO_Status' not in df_rat.columns or df_rat['HO_Status'].dropna().empty:
            df_rat['HO_Status'] = ''
            if pci_col and pci_col in df_rat.columns:
                s_p = pd.to_numeric(df_rat[pci_col], errors='coerce')
                pci_changed = (s_p.diff() != 0) & (s_p.shift(1).notna()) & (s_p > 0)
                df_rat.loc[pci_changed, 'HO_Status'] = 'Success'
        ...
        if arfcn_col not in df_sorted.columns:
            for cand in ['EARFCN', 'Serving_EARFCN', 'NR_ARFCN', 'ARFCN', 'Serving_ARFCN', '[Call & LTE KPI PCell Serving EARFCN(DL)]', '[Call & 5G KPI PCell RF NR-ARFCN]']:
                if cand in df_sorted.columns:
                    arfcn_col = cand
                    break
            else:
                arfcn_col = None
        beam_col = 'Serving_SSB_Idx' if 'Serving_SSB_Idx' in df_sorted.columns else None
        ...
        for idx, row in df_sorted.iterrows():
            ...
            if status in ['MEAS_REPORT', 'MeasurementReport'] or row.get('Event') == 'eventA3':
                ...
                if src_pci is None and idx > 0:
                    src_pci = self._safe_int(df_sorted.loc[idx - 1, pci_col]) if (pci_col and pci_col in df_sorted.columns) else None
                    src_arfcn = self._safe_int(df_sorted.loc[idx - 1, arfcn_col]) if (arfcn_col and arfcn_col in df_sorted.columns) else None
                    src_beam = self._safe_int(df_sorted.loc[idx - 1, beam_col]) if (beam_col and beam_col in df_sorted.columns) else None

                if src_pci is not None and target_pci is not None and src_pci != target_pci:
                    srv_rsrp = last_meas_rsrp.get(src_pci, self._safe_float(row.get('Serving_RSRP'), default=None))
                    tgt_rsrp = last_meas_rsrp.get(target_pci, self._safe_float(row.get('NBR_1_RSRP'), default=None))
                    delta_rsrp = round(tgt_rsrp - srv_rsrp, 1) if (tgt_rsrp is not None and srv_rsrp is not None) else None
                    ho_delay = self._safe_float(row.get('HO_Delay_ms'), default=None)
    ...
    def detect_ping_pong_sessions(
        ...
        # Merge overlapping or contiguous multi-cell ping pong sessions (3 Bounded Guards)
        merged = []
        raw_ping_pongs.sort(key=lambda x: x['start_dt'])

        curr = raw_ping_pongs[0]
        for nxt in raw_ping_pongs[1:]:
            curr_cells = {t['src_pci'] for t in curr['transitions']} | {t['tgt_pci'] for t in curr['transitions']}
            nxt_cells = {t['src_pci'] for t in nxt['transitions']} | {t['tgt_pci'] for t in nxt['transitions']}
            combined_cells = curr_cells | nxt_cells

            gap_sec = (nxt['start_dt'] - curr['end_dt']).total_seconds()
            
            # Guard 1: Dwell time guard - allow contiguous ping-pong chaining up to 10.0s gap
            can_merge = (gap_sec <= 10.0)
        ...
        for sess in merged:
            ...
            valid_rsrps = [t['srv_rsrp'] for t in sess['transitions'] if t.get('srv_rsrp') is not None]
            min_rsrp = min(valid_rsrps) if valid_rsrps else None
            valid_deltas = [t['delta_rsrp'] for t in sess['transitions'] if t.get('delta_rsrp') is not None]
            avg_delta = round(float(np.mean(valid_deltas)), 1) if valid_deltas else None

            if hop_count >= 4 and total_dur <= 60.0:
                severity = "HIGH"
                summary = f"{rat} 다발 핑퐁 ({hop_count}회, {pci_chain_str})"
            elif hop_count == 3:
                severity = "MED"
            ...
            final_episodes.append({
                ...
                'grade': severity,
                'summary': summary,
                'root_cause': f"서빙 셀과 인접 셀 간 신호 세기/품질 중첩 및 핸드오버 마진 부족으로 잦은 기지국 천이 발생 ({total_dur}초간 {hop_count}회 천이)",
                'symptoms': f"기지국 간({pci_chain_str}) 잦은 왕복 핸드오버로 인한 시그널링 부하 및 무선 채널 불안정 발생",
                'causal_fragment': {
                    'trigger_cause': f"기지국 간({pci_chain_str}) 신호 중첩 및 히스테리시스/오프셋 마진 부족으로 {total_dur}초간 {hop_count}회에 걸쳐 왕복 핑퐁 핸드오버 발생",
                    'link_instability': f"{total_dur}초간 {hop_count}회 연속 핸드오버로 무선 품질 불안정 지속"
                },
                'lat': sess.get('lat'),
                'lon': sess.get('lon')
            })
    ...
    def detect_unhandled_ho_requests(self, df_mob: pd.DataFrame, rat: str = 'NR') -> List[Dict[str, Any]]:
        ...
        for s in raw_streaks:
            ...
            s['rule_id'] = 'DIAG_M_02'
            s['domain'] = '01_MOBILITY'
            s['rat'] = rat
            s['time_stamp'] = s['start_ts']
            s['t_start'] = s['start_dt']
            s['t_end'] = s['end_dt']
            s['pci'] = s['srv_pci']
            ...
            s['title'] = s['summary']
            s['badge_label'] = s['summary']
            s['root_cause'] = f"단말이 타겟 셀(PCI {s['nbr_pci']})에 대해 A3 Measurement Report를 총 {cnt}회 전송했으나 기지국에서 핸드오버 명령을 미발행하여 {dur}초간 방치됨"
            s['symptoms'] = f"우세 셀이 타겟 기지국으로 전이되었음에도 핸드오버가 지연되어 서빙 셀 간섭 심화 및 무선 품질 저하 발생"
            s['causal_fragment'] = {
                'trigger_cause': f"단말이 타겟 셀(PCI {s['nbr_pci']})에 대해 A3 Measurement Report를 {cnt}회 전송했으나 기지국에서 핸드오버 명령(RRCConnectionReconfiguration)을 미발행하여 핸드오버가 지연/방치됨",
                'interference': f"타겟 셀 전계 유입으로 서빙 셀 신호 간섭 심화"
            }
            unhandled.append(s)
    ...
    def detect_too_late_handovers(
        ...
        for idx, row in reestab_rows.iterrows():
            ...
            if matching_streaks:
                ...
                streak_delta = streak.get('delta_rsrp')
                is_drop = is_rapid_rsrp_drop(streak_delta, dur_sec)
                rlf_cause_text = f"서빙 전계 급락 Δ{streak_delta:.1f}dB" if (is_drop and streak_delta is not None) else "서빙 무선 링크 단절"
                ...
                timeline = [
                    ...
                    f"[{t_fault_str}] T0 🚨 : 🚨 RadioLinkFailure ({rlf_cause_text} / RLF 발발)",
                ...
                too_late_episodes.append({
                    ...
                    'srv_rsrp': streak.get('last_srv_rsrp') if streak.get('last_srv_rsrp') is not None else streak.get('first_srv_rsrp'),
                    'target_rsrp': streak.get('last_nbr_rsrp') if streak.get('last_nbr_rsrp') is not None else streak.get('first_nbr_rsrp'),
                    'rep_cnt': rep_cnt,
                    'dur_sec': dur_sec,
    ...
    def detect_pci_collisions(
        ...
        - Requires BOTH departure time >= min_departure_seconds (default 5.0s)
          AND physical separation distance >= distance_threshold_km (default 2.0km).
        - Severity:
          * HIGH: Distance >= 4.0 km
          * MED : 2.0 km <= Distance < 4.0 km
          * LOW : Distance >= 2.0 km (default)
        """
            ...
            if pci in pci_history:
                ...
                if time_gap >= min_departure_seconds:
                    ...
                    if 2.0 <= dist_km < 25.0:
                        ...
                        prev_time_str = prev_seg['end_dt'].strftime('%H:%M:%S')
                        curr_time_str = seg['start_dt'].strftime('%H:%M:%S')
                        summary = f"{rat_tag} 중복 PCI 발췌 (PCI {pci}, {dist_km:.1f}km 이격 | {prev_time_str} 이탈 ➔ {curr_time_str} 재진입)"
                        root_cause = (
                            f"동일 PCI {pci}를 사용하는 기지국이 물리적 거리 {dist_km:.2f}km 이격된 위치에서 재검출됨.\n"
                            f"이전 셀 이탈 시각: {prev_time_str} ➔ 신규 셀 진입 시각: {curr_time_str} (시간차: {time_gap:.1f}초)\n"
                            f"물리적 이격 거리(1.0km ~ 5.0km) 기준 중복 PCI(PCI Collision/Confusion)로 식별되어 핸드오버 모순 및 간섭 위험이 존재함."
                        )
                        symptoms = f"동일 PCI 재진입 (물리적 거리: {dist_km:.2f}km, 시간차: {time_gap:.1f}초)"
    ...
    # -------------------------------------------------------------------------
    # [DIAG_M_06_TAU_PINGPONG] Frequent Tracking Area Update (Ping-Pong TAU)
    # -------------------------------------------------------------------------
    def detect_frequent_tau(
        self,
        df_events: Optional[pd.DataFrame] = None,
        time_window_sec: float = 45.0,
        min_tau_count: int = 3,
        csvs: Optional[Dict[str, str]] = None
    ) -> List[Dict[str, Any]]:
        """
        [DIAG_M_06] Frequent Tracking Area Update (Ping-Pong TAU) Detector.
        - Detects excessive TAU requests (>= min_tau_count within time_window_sec) due to TAC border oscillations.
        """
        if df_events is None or df_events.empty:
            if csvs:
                ed_path = csvs.get('EVENT_DETAIL') or csvs.get('EVENT')
                if ed_path and os.path.exists(ed_path):
                    try:
                        df_events = pd.read_csv(ed_path, encoding='utf-8', low_memory=False)
                    except Exception:
                        try:
                            df_events = pd.read_csv(ed_path, encoding='cp949', low_memory=False)
                        except Exception:
                            df_events = None
        ...
        df_ev = df_events.copy()
        ts_col = 'TIME_STAMP' if 'TIME_STAMP' in df_ev.columns else ('Time' if 'Time' in df_ev.columns else None)
        if not ts_col:
            return []

        df_ev['_dt'] = pd.to_datetime(df_ev[ts_col], errors='coerce')
        df_ev = df_ev.dropna(subset=['_dt']).sort_values('_dt').reset_index(drop=True)

        detail_col = None
        for col in ['[Call & Voice Call Event Detail Code1]', 'Event', 'Message_Type', 'Detail', 'Message']:
            if col in df_ev.columns:
                detail_col = col
                break
        ...
        tau_rows = df_ev[df_ev[detail_col].astype(str).str.contains(r'Tracking area update request', case=False, na=False)]
        if len(tau_rows) < min_tau_count:
            return []

        tau_episodes = []
        tau_list = tau_rows.to_dict('records')
        n = len(tau_list)
        ...
        while i < n:
            ...
            j = i + 1
            while j < n:
                gap = (tau_list[j]['_dt'] - cluster[-1]['_dt']).total_seconds()
                if gap <= 15.0:
                    cluster.append(tau_list[j])
                    j += 1
                else:
                    break

            total_dur = (cluster[-1]['_dt'] - cluster[0]['_dt']).total_seconds()
            tau_cnt = len(cluster)

            if tau_cnt >= min_tau_count and total_dur <= time_window_sec:
                start_dt = cluster[0]['_dt']
                end_dt = cluster[-1]['_dt']
                summary = f"LTE TAC 경계 핑퐁 및 빈번한 TAU 발생 (TAU {tau_cnt}회 / {round(total_dur, 1)}초간)"
                cause = f"서로 다른 TAC 경계 인근 주행 중 {round(total_dur, 1)}초 동안 Tracking Area Update Request가 {tau_cnt}회 연속 발생 (과다 시그널링 부하 초래)"
                title = f"TAC 핑퐁 (빈번한 TAU {tau_cnt}회)"
                tau_episodes.append({
                    'title': title,
                    'diag_code': 'DIAG_M_06_TAU_PINGPONG',
                    'rule_id': 'DIAG_M_06',
                    'rat': 'LTE',
    ...
    def diagnose_mobility_all(
        ...
        """
        all_episodes = []
        if df_mob is None and df_timeline is not None:
            df_mob = df_timeline
        ...
        if df_events is not None and not df_events.empty:
            ...
            # DIAG_M_06: Frequent TAU
            tau_eps = self.detect_frequent_tau(df_events)
            all_episodes.extend(tau_eps)
