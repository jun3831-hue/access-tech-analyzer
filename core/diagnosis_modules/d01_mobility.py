...
class MobilityDetector:
    ...
    def extract_mobility_transitions(self, df_mob: pd.DataFrame, rat: str = 'NR') -> List[Dict[str, Any]]:
        ...
        if pci_col not in df_rat.columns and 'pci' in df_rat.columns:
            pci_col = 'pci'

        if 'HO_Status' not in df_rat.columns or df_rat['HO_Status'].dropna().empty:
            df_rat['HO_Status'] = ''
            if pci_col in df_rat.columns:
    ...
    def detect_ping_pong_sessions(
        ...
        for nxt in raw_ping_pongs[1:]:
            ...
            # Guard 1: Dwell time guard - if UE dwells >= 3.0s, split into distinct ping-pong session
            # Guard 2: Overlapping cell topology (must share cells)
            # Guard 3: Max distinct cell limit <= 3 to prevent linear driving snowball chaining across different cell clusters
            can_merge = (gap_sec < 3.0) and bool(curr_cells & nxt_cells) and (len(combined_cells) <= 3)
