...
from core.diagnosis_modules.d06_voice_ims import VoiceImsDetector
...
class DiagnosisReporter:
    ...
    def detect_frequent_tau(self, df_events=None, time_window_sec=45.0, min_tau_count=3, csvs=None):
        return self.d01.detect_frequent_tau(df_events=df_events, time_window_sec=time_window_sec, min_tau_count=min_tau_count, csvs=csvs)

    def diagnose_voice_all(self, df_timeline=None, csvs=None, df_events=None, network_mode='LTE'):
        return self.d06.diagnose_voice_all(df_timeline=df_timeline, csvs=csvs, df_events=df_events, network_mode=network_mode)
    ...
    def generate_full_text_report(
        ...
        # Detect DIAG_M_06 TAU Ping-Pong (Frequent TAU)
        tau_pingpong = self.detect_frequent_tau(df_events=None, time_window_sec=45.0, min_tau_count=3, csvs=csvs)
        ...
        if csvs:
            try:
                ...
                mimo_rank_issues = phy_det.detect_mimo_rank_restrictions(csvs=csvs, all_l3=all_l3, df_call_summary=df_call_sum)
                crc_issues = phy_det.detect_continuous_crc_errors(csvs=csvs, all_l3=all_l3, df_call_summary=df_call_sum)
                phy_res = {
        ...
        # Domain 06: Voice / IMS Session Diagnostics
        voice_episodes = self.d06.diagnose_voice_all(
            df_timeline=df_timeline,
            csvs=csvs,
            df_events=None,
        ...
        # Merge voice episodes into combined episodes
        combined_episodes = list(episodes)
        for ve in voice_episodes:
            ve_ts = str(ve.get('time_stamp', ''))
            already_merged = False
            for ep in combined_episodes:
                ep_ts = str(ep.get('t_start', ''))
                if ve_ts and ve_ts[:19] in ep_ts:
                    ep['has_volte_drop'] = True
                    ep['mute_duration_sec'] = ve.get('mute_duration_sec', 0)
                    if ve.get('story_steps'):
                        ep['story_steps'] = ve['story_steps']
                    already_merged = True
                    break
            if not already_merged:
                combined_episodes.append(ve)

        combined_episodes.sort(key=lambda x: str(x.get('t_start', x.get('time_stamp', ''))))
        self.last_episodes = combined_episodes
        causal_block = self.tracer.format_incident_report(combined_episodes)
        ...
        # ---------------------------------------------------------------------
        # DOMAIN 01-D: TAC PING-PONG & FREQUENT TAU ANALYSIS
        # ---------------------------------------------------------------------
        lines.append(divider)
        lines.append(f" [도메인 01-D] TAC 핑퐁 및 빈번한 TAU (Tracking Area Update) 분석")
        lines.append(f" • 총 검출된 TAC 핑퐁 TAU 결함: {len(tau_pingpong)}건")
        lines.append(divider)

        lines.append("■ [DIAG_M_06_TAU_PINGPONG] 30초 이내 빈번한 Tracking Area Update 검출 결과")
        if not tau_pingpong:
            lines.append("  ✔ TAC 경계 왕복 및 단시간 빈번한 TAU 미발생 (정상 페이징/위치 등록 상태)")
            lines.append("")
        else:
            for t_idx, t in enumerate(tau_pingpong, 1):
                start_ts = t.get('start_ts', str(t.get('time_stamp', '')))
                sev = t.get('severity', 'MED')
                lines.append(f"  ▶ [TAC 핑퐁 결함 #{t_idx}] {start_ts} | {t.get('summary', '')} | 심각도: {sev}")
                lines.append(f"    • 분석 소견: {t.get('root_cause', '')}")
                lines.append("")
        ...
        # ---------------------------------------------------------------------
        # DOMAIN 06: VoLTE / VoNR VOICE QUALITY & IMS SESSION ANALYSIS
        # ---------------------------------------------------------------------
        lines.append(divider)
        lines.append(f" [도메인 06] 🎙️ VoLTE / VoNR 음성 품질 및 IMS 세션 분석")
        v_drops = [v for v in voice_episodes if v.get('diag_code') == 'DIAG_V_02_RTP_DROP']
        v_mutes = [v for v in voice_episodes if v.get('diag_code') == 'DIAG_V_03_AUDIO_MUTE']
        v_setups = [v for v in voice_episodes if v.get('diag_code') == 'DIAG_V_01_SETUP_FAIL']
        v_jitters = [v for v in voice_episodes if v.get('diag_code') == 'DIAG_V_04_JITTER_LOSS']
        lines.append(f" • 총 음성 호 단절: {len(v_drops)}건  |  통화 묵음(Mute): {len(v_mutes)}건  |  호 수립 실패: {len(v_setups)}건  |  지터/손실: {len(v_jitters)}건")
        lines.append(divider)

        # DIAG_V_02_RTP_DROP
        lines.append("■ [DIAG_V_02_RTP_DROP] VoLTE / VoNR 호 비정상 절단 (RTP Drop - Bye) 검출 결과")
        if not v_drops:
            lines.append("  ✔ 통화 중 RTP 패킷 단절 및 강제 SIP BYE 종료 미발생 (정상 통화 완료)")
            lines.append("")
        else:
            for d_idx, d in enumerate(v_drops, 1):
                t_str = d.get('time_stamp', '')
                lines.append(f"  ▶ [음성 호 절단 #{d_idx}] {t_str} | {d.get('summary', '')} | 심각도: {d.get('severity', 'HIGH')}")
                lines.append(f"    • 분석 소견: {d.get('root_cause', '')}")
                if d.get('story_steps'):
                    lines.append("    • 호 단절 진행 경과:")
                    for st in d['story_steps']:
                        lines.append(f"      - {st}")
                lines.append("")

        lines.append(sub_divider)

        # DIAG_V_03_AUDIO_MUTE
        lines.append("■ [DIAG_V_03_AUDIO_MUTE] 하향 음성 묵음 (Audio Mute >= 2.0s) 검출 결과")
        if not v_mutes:
            lines.append("  ✔ 통화 중 2.0초 이상의 하향 음성 패킷 단절/묵음 미발생 (양호한 음질 유지)")
            lines.append("")
        else:
            for m_idx, m in enumerate(v_mutes, 1):
                lines.append(f"  ▶ [통화 묵음 #{m_idx}] {m.get('time_stamp', '')} | {m.get('summary', '')} | 심각도: {m.get('severity', 'MED')}")
                lines.append(f"    • 분석 소견: {m.get('root_cause', '')}")
                lines.append("")
