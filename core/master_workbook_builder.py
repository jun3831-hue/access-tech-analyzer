...
class MasterWorkbookBuilder:
    ...
    def build_master_consolidated_excel(
        ...
        # 1-1. Dedicated scenario per_call summary sheets (for legacy & cross-tool compatibility)
        hdr_font_call = Font(name="Malgun Gothic", size=9, bold=True, color="FFFFFF")
        fill_hdr_call = PatternFill(start_color="1E293B", end_color="1E293B", fill_type="solid")
        thin_side_call = Side(border_style="thin", color="CBD5E1")
        border_call = Border(left=thin_side_call, right=thin_side_call, top=thin_side_call, bottom=thin_side_call)

        for port_key, pdata in port_results.items():
            summaries = pdata.get('summaries', {})
            for sc_key in ['DL', 'UL', 'Ping', 'Voice']:
                df_sc = summaries.get(sc_key)
                if df_sc is not None and isinstance(df_sc, pd.DataFrame) and not df_sc.empty:
                    sheet_title = f"{port_key}_{sc_key}_per_call"
                    ws_sc = wb.create_sheet(title=sheet_title)
                    for r_i, row in enumerate(dataframe_to_rows(df_sc, index=False, header=True), start=1):
                        for c_i, val in enumerate(row, start=1):
                            cell = ws_sc.cell(row=r_i, column=c_i, value=val)
                            cell.border = border_call
                            if r_i == 1:
                                cell.font = hdr_font_call
                                cell.fill = fill_hdr_call
