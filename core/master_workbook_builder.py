...
class MasterWorkbookBuilder:
    ...
    HEADER_TO_SOURCE_COLS = {
        ...
        '시나리오': ['Traffic_Model', '시나리오', 'Scenario', 'Scenario Name'],
        '망 모드': ['Network_Mode', '망 모드', 'System Mode', 'Network Type'],
        '호 상태': ['Call_Phase', '[Call & Call State Call Event]', '호 상태', '호 상태/구간', 'Status'],
        ...
        '5G MAC T-put per RB (kbps)': ['NR_MAC_Tput_Per_RB', '[Call & Qualcomm 5G-NR UL/DL Info Summary(In-Traffic) PCell MAC Throughput per RB DL MAC Throughput Per RB [kbps]]', 'DL MAC Throughput Per RB', '5G MAC T-put per RB (kbps)'],
        '5G MAC T-put per RB': ['NR_MAC_Tput_Per_RB', '[Call & Qualcomm 5G-NR UL/DL Info Summary(In-Traffic) PCell MAC Throughput per RB DL MAC Throughput Per RB [kbps]]', 'DL MAC Throughput Per RB', '5G MAC T-put per RB'],
    ...
    def _extract_1sec_rows(self, df_tl: pd.DataFrame, headers: List[str]) -> List[Dict[str, Any]]:
        ...
        for _, r in df_tl.iterrows():
            ...
            for h in headers:
                ...
                elif h in ['호 상태', '호 상태 (성공률)']:
                    row_dict[h] = str(r.get('Call_Phase', 'Traffic')) if pd.notna(r.get('Call_Phase')) else 'Traffic'
                elif h == '시나리오':
                    sc_attr = df_tl.attrs.get('Traffic_Model') or r.get('Traffic_Model')
                    row_dict[h] = sc_attr or 'DL'
                elif h == '망 모드':
                    net_attr = df_tl.attrs.get('Network_Mode') or r.get('Network_Mode')
                    row_dict[h] = net_attr or 'LTE'
                elif h == '호 방향':
                    row_dict[h] = str(df_tl.attrs.get('Direction', r.get('호 방향', 'MO')))
                elif h == '호 릴리즈 원인 (Cause)':
                    row_dict[h] = str(r.get('호 릴리즈 원인 (Cause)', r.get('Release_Cause', '-')))
                elif h == '통화 지속 시간 (초)':
                    row_dict[h] = r.get('통화 지속 시간 (초)', r.get('Duration', 1))
                else:
    ...
    def _extract_call_rows(self, df_tl: pd.DataFrame, headers: List[str], scenario: str) -> List[Dict[str, Any]]:
        ...
        for c_idx, (call_label, grp) in enumerate(call_groups):
            ...
            dur_sec = round((ts_s.max() - ts_s.min()).total_seconds(), 1) if not ts_s.empty else 0.0
            ...
            is_drop = False
            if 'HO_Status' in grp.columns and (grp['HO_Status'].astype(str).str.contains('Fail|Drop|RLF|Reject', case=False)).any():
                is_drop = True
            call_status_str = "Drop" if is_drop else "Success"

            sc_val = df_tl.attrs.get('Traffic_Model', scenario)
            net_val = df_tl.attrs.get('Network_Mode', 'LTE')

            row_dict['시간 (구간)'] = f"{st_str} ~ {et_str}"
            row_dict['시간 구간'] = f"{st_str} ~ {et_str}"
            row_dict['호 번호'] = call_label
            row_dict['시나리오'] = sc_val
            row_dict['망 모드'] = net_val
            row_dict['호 상태'] = call_status_str
            row_dict['호 상태 (성공률)'] = call_status_str
            row_dict['호 방향'] = df_tl.attrs.get('Direction', 'MO')
            row_dict['호 릴리즈 원인 (Cause)'] = "Drop (RLF)" if is_drop else "Normal Release"
            row_dict['통화 지속 시간 (초)'] = dur_sec

            for h in headers:
                if h in ['시간 (구간)', '시간 구간', '호 번호', '시나리오', '망 모드', '호 상태', '호 상태 (성공률)', '호 방향', '호 릴리즈 원인 (Cause)', '통화 지속 시간 (초)']:
                    continue
                elif h in ['Lon', 'Lat']:
    ...
    def _extract_total_summary_row(self, df_tl: pd.DataFrame, call_rows: List[Dict[str, Any]], headers: List[str]) -> Dict[str, Any]:
        ...
        success_calls = sum(1 for c in call_rows if 'Success' in str(c.get('호 상태', c.get('호 상태 (성공률)', '')))) if call_rows else total_calls
        ...
        sc_val = df_tl.attrs.get('Traffic_Model', 'Unknown')
        net_val = df_tl.attrs.get('Network_Mode', 'LTE')
        ...
        row_dict['시나리오'] = sc_val
        row_dict['망 모드'] = net_val
        row_dict['호 상태'] = f"{success_calls} / {total_calls} ({succ_rate}%)"
        row_dict['호 상태 (성공률)'] = f"{success_calls} / {total_calls} ({succ_rate}%)"
        row_dict['호 방향'] = df_tl.attrs.get('Direction', 'MO')
        row_dict['호 릴리즈 원인 (Cause)'] = "-"
        row_dict['통화 지속 시간 (초)'] = dur_sec
    ...
    def _detect_sub_traffic_models(self, df_tl: pd.DataFrame, scenario: str) -> Dict[str, pd.DataFrame]:
        ...
        For standalone scenarios (DL only, UL only, Voice only), strict 1:1 single sheet mapping is enforced.
        """
        sc_up = str(scenario).upper()
        ...
        if df_tl is None or df_tl.empty:
            return {sc_clean: pd.DataFrame()}

        # Standalone scenarios MUST NOT be fragmented by background packets
        if sc_clean in ['VoLTE', 'UL']:
            return {sc_clean: df_tl}

        # SST or Multi-scenario partitioning only if explicitly SST
        sub_dfs = {}
        if 'SST' in sc_up and 'Call_Phase' in df_tl.columns:
        ...
        if any(k in sc_up for k in ['VOICE', 'VOLTE', 'VONR']):
            ...
            if df_tl is not None and not df_tl.empty:
                for c in df_tl.columns:
                    if 'mos' in str(c).lower() and df_tl[c].dropna().count() > 0:
                        s_m = pd.to_numeric(df_tl[c], errors='coerce').dropna()
                        if not s_m.empty and s_m.max() > 0:
                            mos_col = c
                            break
            mos_header = 'MOS 평균' if mos_col else 'MOS 평균 (미측정)'
            mos_min_header = 'MOS 최소 (Min)' if mos_col else 'MOS 최소 (미측정)'
            headers = [
                '호 번호', '호 방향', '시간 (구간)', '시나리오', '망 모드', '호 상태', '호 릴리즈 원인 (Cause)', '통화 지속 시간 (초)',
                mos_header, mos_min_header, '음성 코덱 (Codec)',
    ...
    def build_master_consolidated_excel(
        ...
        # 0. First sheet directly begins with the first terminal traffic sheet (e.g. M1_DL) - No artificial '01_통합_요약' sheet
        ...
        # 2. Clean up if default_sheet was untouched
        if is_first_sheet and default_sheet in wb.worksheets:
            default_sheet.title = "Empty_Result"
