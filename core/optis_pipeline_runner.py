...
class OptisPipelineRunner:
    ...
    def run(self, zip_paths: List[str], log_cb=None) -> Dict[str, Any]:
        """
        ...
        with tempfile.TemporaryDirectory(prefix="optis_exec_") as work_dir:
            ...
            for drm_idx, display_name in enumerate(drm_names, 1):
                ...
                _log(f"\n▶ [{display_name}] ({port_key}) 마스터 타임라인 및 L3 시그널링 파싱 중...")
            ...
            for g_key, items in grouped_port_results.items():
                ...
                for pk, p_item in group_port_dict.items():
                    p_rep = p_item.get('txt_rep', '')
                    port_txt_reports[pk] = p_rep
                    combined_txt_reports.append(p_rep)
                ...
                for pk, p_item in group_port_dict.items():
                    ...
                    if not tl.empty:
                        for time_col in ['Time', 'Timestamp', 'time', 'TIME', 'Date', 'D-Time']:
                            if time_col in tl.columns:
                                valid_ts = tl[time_col].dropna()
                                if not valid_ts.empty:
                                    meas_date_str = str(valid_ts.iloc[0])
                                    break
                    if meas_date_str:
                        break
                ...
                final_results[g_key] = {
                    ...
                    'total_pts': total_pts,
                    'measurement_date': meas_date_str,
                    'port_results': group_port_dict
                }
                _log(f"✅ [{g_key}] 분석 완료! (에피소드: {total_episodes}건, 측정점: {total_pts}점)")

            return final_results

    def run_all(self, file_paths: List[str], log_cb=None) -> Dict[str, Any]:
        """
        Runs analysis on a mixed list of .zip or .drm files.
        Converts .drm to .zip automatically using OptisExtractor if needed.
        """
        def _log(msg: str):
            if log_cb:
                try:
        ...
        drm_files = [f for f in file_paths if f.lower().endswith('.drm')]

        if drm_files:
            _log(f"⚙️ {len(drm_files)}개 DRM 파일 감지됨 ➔ OPTis-S4 연동 CSV 추출 및 ZIP 패키징 진행...")
            with tempfile.TemporaryDirectory(prefix="optis_drm_") as tmp_d:
                from core.optis_extractor import OptisExtractor
                settings_dir = os.path.join(project_root, "settings")
                fav_files = glob.glob(os.path.join(settings_dir, "*.fav"))
                if not fav_files:
                    fav_files = glob.glob(os.path.join(project_root, "SETTINGS", "*.fav"))

                extractor = OptisExtractor(cache_dir=os.path.join(tmp_d, "cache"))
                extracted = extractor.extract(drm_files, fav_files, log_cb=log_cb)
                for d_name, c_dict in extracted.items():
                    if c_dict:
                        zp = os.path.join(tmp_d, f"{d_name}.zip")
                        with zipfile.ZipFile(zp, 'w', zipfile.ZIP_DEFLATED) as zf:
                            for alias, fp in c_dict.items():
                                if fp and os.path.exists(fp):
                                    zf.write(fp, os.path.basename(fp))
                        zip_files.append(zp)
                return self.run(zip_files, log_cb=log_cb)
        else:
            return self.run(zip_files, log_cb=log_cb)
