...
class DLRSParser:
    ...
    def parse(self, input_data: Union[pd.DataFrame, Dict[str, pd.DataFrame]]) -> pd.DataFrame:
        ...
        for _, r in df_raw.iterrows():
            ...
            pci = self._safe_int(r.get('LTE_PCI', r.get('NR_PCI')), default=None)
            action = str(r.get('ACTION', 'MODIFY')).upper()

            res_id = self._safe_int(r.get('nzp-CSI-RS-ResourceId', r.get('zp-CSI-RS-ResourceId')), default=None)
            ports = str(r.get('nrofPorts', 'p1'))
            sym = self._safe_int(r.get('firstOFDMSymbolInTimeDomain'), default=None)
            period = 160 if pd.notna(r.get('slots160')) else (320 if pd.notna(r.get('slots320')) else None)
            slot_offset = self._safe_int(r.get('slots160', r.get('slots320')), default=None)
