        ...
        for hdr, body in blocks:
            ...
            if 'measConfig' in body_text or 'reportConfig' in body_text or 'measObject' in body_text:
                ...
                for r_id in re.findall(r'reportConfigId\s*[:=]?\s*(\d+)', body_text):
                    ...
                    ttt_str = m_ttt.group(1) if m_ttt else "미수신"
                    rep_map[int(r_id)] = {
                        'ev_name': ev_name,
                        'off_str': off_str,
                        'hys_str': hys_str,
                ...
                for m_id, o_id, r_id in re.findall(r'measId\s*[:=]?\s*(\d+).*?measObjectId\s*[:=]?\s*(\d+).*?reportConfigId\s*[:=]?\s*(\d+)', body_text, re.DOTALL):
                    ...
                    earfcn = obj_map.get(oid_int, '미설정')
                    r_info = rep_map.get(rid_int)
                    if r_info:
                        ev_n = r_info['ev_name']
                        ...
                        key_name = f"[ReportConfig #{rid_int}] (EARFCN {earfcn})"
                        struct_registry['01. 이동성 / 핸드오버 (MeasConfig)'][key_name][current_serving_pci] = "[Config 파라미터 미수신]"
