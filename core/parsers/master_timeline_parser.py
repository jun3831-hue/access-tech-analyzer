...
class MasterTimelineParser:
        ...
        if df_ed is not None and not df_ed.empty and 'TIME_STAMP' in df_ed.columns:
            ...
            for sec_str, grp in df_temp.groupby('__sec', sort=False):
                ...
                for _, r in grp.iterrows():
                    ...
                    # SIP check
                    if any(k in full_txt for k in ['sip', 'bye', 'invite', '200 ok', '180', 'ringing', 'update']):
                        ...
                        elif 'update' in full_txt:
                            msg_name = f"SIP UPDATE {dir_tag}".strip()
                        else:
                            msg_name = f"SIP: {c1_val or info_val or st_val}".strip()
                        if msg_name not in sip_items:
                            sip_items.append(msg_name)

                    # L3 / NAS / Radio Event check (Full ingestion: exclude only periodic noise)
                    ...
                    if t_col and m_col:
                        ...
                        for sec_str, grp in df_mb.groupby('_sec', sort=False):
                            ...
                            for m in grp[m_col].dropna().astype(str).tolist():
                                ...
                                if m_lower in ['masterinformationblock', 'mib', 'systeminformationblocktype1']:
                                    continue
                                matched_canon = None
    ...
    def build_master_timeline(
        ...
        # Merge all frames preserving 100% of all original and alias columns
        if sub_kpi is not None and not sub_kpi.empty and 'TIME_STAMP' in sub_kpi.columns:
            ...
            df_1hz = sub_kpi.dropna(subset=['clean_sec']).groupby('clean_sec').last().reset_index()
            ...
            # Fallback if no KPI table exists
        ...
        df_1hz = df_1hz.drop(columns=['sec_tag'], errors='ignore')
