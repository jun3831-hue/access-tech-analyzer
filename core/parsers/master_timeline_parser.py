                ...
                if df_mb is not None and not df_mb.empty:
                    ...
                    if t_col and m_col:
                        ...
                        canonical_patterns = [
                            ...
                            ('reestablishment', 'RRCConnectionReestablishment'),
                            ...
                            ('connectionreject', 'RRCConnectionReject'),
                        ...
                        for sec_str, grp in df_mb.groupby('_sec', sort=False):
                            ...
                            for m in grp[m_col].dropna().astype(str).tolist():
                                ...
                                # Ignore periodic broadcast, paging and internal transfer wrapper noise
                                if any(k in m_lower for k in [
                                    'masterinformationblock', 'mib', 'systeminformationblocktype1', 'sib1',
                                    'paging', '__paging', 'pcch',
                                    ...
                                    'systeminformation', '__systeminformation',
                                    'security protected nas message', 'emm information'
                                ]):
                                    continue
                                matched_canon = None
    ...
    def build_master_timeline(
        ...
        if df_kpi is not None and not df_kpi.empty and 'TIME_STAMP' in df_kpi.columns:
            ...
            for target, kws in [
                ...
                ('MOS', ['[Call & MOS P863(POLQA)]', 'MOS P863', 'MOS Result', 'P863(POLQA)', 'POLQA', 'MOS']),
                ('NR_MAC_Tput_Per_RB', ['[Call & Qualcomm 5G-NR UL/DL Info Summary(In-Traffic) PCell MAC Throughput per RB DL MAC Throughput Per RB [kbps]]', 'DL MAC Throughput Per RB', '5G MAC T-put per RB', 'MAC Throughput per RB'])
            ]:
        ...
        # 4. Event & Event Detail (AutoCallSummary / Voice Call Service / LTE/NR Events)
        ...
        sub_event = None
        if df_event is not None and not df_event.empty and 'TIME_STAMP' in df_event.columns:
            ev_col_names = []
            for col_c in ['[Call & Event LTE Events]', '[Call & Event 5G-NR/LTE Events]', 'Event', 'Events']:
                if col_c in df_event.columns:
                    ev_col_names.append(col_c)
            if ev_col_names:
                df_ev_copy = df_event.copy()
                df_ev_copy['clean_sec'] = pd.to_datetime(df_ev_copy['TIME_STAMP'], errors='coerce').dt.floor('s')
                ev_by_sec_dict = {}
                for s_ts, grp in df_ev_copy.dropna(subset=['clean_sec']).groupby('clean_sec'):
                    sec_evs = []
                    for c_name in ev_col_names:
                        for val in grp[c_name].dropna().astype(str).tolist():
                            v_clean = val.strip()
                            if v_clean and v_clean not in sec_evs:
                                sec_evs.append(v_clean)
                    if sec_evs:
                        ev_by_sec_dict[s_ts] = " ➔ ".join(sec_evs)
                if ev_by_sec_dict:
                    sub_event = pd.DataFrame(list(ev_by_sec_dict.items()), columns=['clean_sec', 'Event'])
                    sub_event['Source_Type'] = 'Event'
                    time_series_frames.append(sub_event)
        ...
        if sub_event is not None and not sub_event.empty and 'clean_sec' in sub_event.columns:
            df_1hz = pd.merge(df_1hz, sub_event[['clean_sec', 'Event']], on='clean_sec', how='left')
        ...
        df_1hz['L3_Signaling'] = df_1hz['sec_tag'].map(l3_by_sec).fillna('')

        # If L3_Signaling is empty but Event exists, combine appropriately
        if 'Event' in df_1hz.columns:
            df_1hz['Event'] = df_1hz['Event'].fillna('')
            m_empty_l3 = (df_1hz['L3_Signaling'] == '') & (df_1hz['Event'] != '')
            df_1hz.loc[m_empty_l3, 'L3_Signaling'] = df_1hz.loc[m_empty_l3, 'Event']
        else:
            df_1hz['Event'] = ''

        # Specialize MeasurementReport into eventA3 MeasurementReport if eventA3 occurred in this second
        if 'Event' in df_1hz.columns and 'L3_Signaling' in df_1hz.columns:
            m_a3_sec = df_1hz['Event'].astype(str).str.contains('eventA3|A3', case=False, na=False)
            df_1hz.loc[m_a3_sec, 'L3_Signaling'] = df_1hz.loc[m_a3_sec, 'L3_Signaling'].astype(str).str.replace(
                'MeasurementReport', 'eventA3 MeasurementReport', regex=False
            )
        ...
        if call_intervals:
            for interval in call_intervals:
                ...
                # Bind MOS per call interval (POLQA MOS is evaluated per call or speech period)
                if 'MOS' in df_1hz.columns:
                    call_mos = pd.to_numeric(df_1hz.loc[c_mask, 'MOS'], errors='coerce')
                    if call_mos.notna().any():
                        df_1hz.loc[c_mask, 'MOS'] = call_mos.ffill().bfill()

            # Rows outside any call interval should remain NaN for MOS
            if 'MOS' in df_1hz.columns:
                in_any_call = pd.Series(False, index=df_1hz.index)
                for interval in call_intervals:
                    in_any_call |= (df_1hz['TIME_STAMP'] >= interval['start']) & (df_1hz['TIME_STAMP'] <= interval['end'])
                df_1hz.loc[~in_any_call, 'MOS'] = np.nan
        else:
            # Fallback continuous
            if traffic_model.startswith('VOICE'):
                df_1hz['Call_Phase'] = 'VOICE_Traffic'
                if 'MOS' in df_1hz.columns:
                    call_mos = pd.to_numeric(df_1hz['MOS'], errors='coerce')
                    if call_mos.notna().any():
                        df_1hz['MOS'] = call_mos.ffill().bfill()
