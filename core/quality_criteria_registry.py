...
def get_bandwidth_based_throughput_criteria(
    ...
    if mode == 'LTE':
        ...
        title = f'🚀 LTE Total MAC ({int(eff_lte)}MHz)'
        m_id = 'lte_mac'
    elif mode == 'SA':
        v_target = eff_nr * 8.0
        title = f'🚀 NR Total MAC ({int(eff_nr)}MHz)'
        m_id = 'nr_mac'
    else:  # NSA
        tot_bw = eff_nr + eff_lte
        v_target = (eff_nr * 8.0) + (eff_lte * 5.0)
        title = f'🚀 Total PDCP ({int(tot_bw)}MHz)'
        m_id = 'pdcp_total'
...
def get_all_map_criteria_dict(
    ...
    if net_mode:
        dyn_crit = get_bandwidth_based_throughput_criteria(net_mode, lte_bw or 60.0, nr_bw or 100.0)
        m_id = dyn_crit['metric_id']
        base_dict[m_id] = dyn_crit
        base_dict['fourth'] = dyn_crit
        base_dict['app_tp'] = dyn_crit
    return base_dict
