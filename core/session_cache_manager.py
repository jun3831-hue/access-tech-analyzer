...
def extract_yymmdd(raw_val: Any, fallback_dt: Optional[Any] = None) -> str:
    """
    ...
    if raw_val is None or str(raw_val).strip() == "" or str(raw_val).strip().lower() == "none":
        if fallback_dt is not None:
            return extract_yymmdd(fallback_dt)
    ...
    # 2. Match 6-digit YYMMDD (24~29)
    m_yymmdd = re.search(r'(?:^|[^0-9])(2[4-9](?:0[1-9]|1[0-2])(?:0[1-9]|[12]\d|3[01]))(?:[^0-9]|$)', s)
    if m_yymmdd:
        return m_yymmdd.group(1)

    # 3. Match 4-digit MMDD like 0623 -> Combine with current year (e.g. 26)
    m_mmdd = re.search(r'(?:^|[^0-9])(0[1-9]|1[0-2])([0-3]\d)(?:[^0-9]|$)', s)
    if m_mmdd:
        cur_yy = datetime.now().strftime('%y')
        return f"{cur_yy}{m_mmdd.group(1)}{m_mmdd.group(2)}"
...
class SessionCacheManager:
    ...
    """
    REMOTE_BASE_CACHE_DIR = "/Personal/전광용/DM Agent/sessions"
    ...
    def save_local_session(
        ...
        port_txt_reports: Optional[Dict[str, str]] = None
    ) -> str:
        """
        Saves a session bundle to the local cache directory.
        Supports saving dedicated report_{port_key}.txt for each port alongside consolidated report.txt.
        """
        ...
        # 4. report.txt (consolidated)
        txt_path = os.path.join(session_dir, "report.txt")
        with open(txt_path, 'w', encoding='utf-8', errors='ignore') as f:
            f.write(txt_report)

        # 5. Dedicated individual port reports: report_{port_key}.txt
        if port_txt_reports:
            for pk, p_txt in port_txt_reports.items():
                p_txt_path = os.path.join(session_dir, f"report_{pk}.txt")
                with open(p_txt_path, 'w', encoding='utf-8', errors='ignore') as f:
                    f.write(p_txt)
    ...
    def load_local_session(self, session_name: str) -> Optional[Dict[str, Any]]:
        ...
        # Load individual port reports if present
        port_txt_reports = {}
        for fn in os.listdir(session_dir):
            if fn.startswith("report_") and fn.endswith(".txt"):
                pk = fn.replace("report_", "").replace(".txt", "")
                try:
                    with open(os.path.join(session_dir, fn), 'r', encoding='utf-8', errors='ignore') as f:
                        port_txt_reports[pk] = f.read()
                except Exception:
                    pass
        ...
        return {
            ...
            'port_txt_reports': port_txt_reports,
    ...
    def upload_session_to_remote(
        ...
        if os.path.exists(l_dir):
            for fn in os.listdir(l_dir):
                if fn.startswith("report_") and fn.endswith(".txt") and fn not in files_to_upload:
    ...
    def download_remote_session(
        ...
        if is_sftp and HAS_PARAMIKO:
            try:
                ...
                sftp = ssh.open_sftp()
                try:
                    r_entries = sftp.listdir(r_session_dir)
                    for re_f in r_entries:
                        if re_f.startswith("report_") and re_f.endswith(".txt") and re_f not in files_to_get:
                            files_to_get.append(re_f)
                except Exception:
                    pass
        ...
        port_txt_reports = {}
        for fn, b_val in downloaded_bytes.items():
            if fn.startswith("report_") and fn.endswith(".txt"):
                pk = fn.replace("report_", "").replace(".txt", "")
                port_txt_reports[pk] = b_val.decode('utf-8', errors='ignore')
        ...
        try:
            self.save_local_session(session_name, map_html, excel_bytes, txt_report, meta, port_txt_reports=port_txt_reports)
        except Exception:
            pass
