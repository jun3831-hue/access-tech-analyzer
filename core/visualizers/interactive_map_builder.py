...
class InteractiveMapBuilder:
    ...
    def extract_trajectory_from_timeline(
        ...
        # Throughput (Prioritize raw CSV columns from Master DF)
        ...
        col_nr_mac = self._find_col(df_timeline, [
            ...
            '[Call & 5G KPI PCell Layer1 PDSCH Throughput [Mbps]]',
        ...
        col_dl = self._find_col(df_timeline, [
            '[Call & APP Throughput Info(All Data) All FWD  Throughput (kbps)]',
            '[Call & Speed Test T-put Current App Throughput [Mbps]]',
            'App_DL_Tput', 'App DL 속도 (Mbps)', 'App DL 속도'
        ]) or col_pdcp or col_lte_mac or col_nr_mac
        col_ul = self._find_col(df_timeline, [
            '[Call & APP Throughput Info(All Data) All RVS Throughput (kbps)]',
            'App_UL_Tput', 'App UL 속도 (Mbps)', 'App UL 속도'
        ])
    ...
    def extract_incident_episodes(
        ...
        if episodes:
            for ep_idx, ep in enumerate(episodes, 1):
                ...
                dur_sec = ep.get('duration_sec') or ep.get('dur_sec') or 10.0
                ...
                # Target PCI and RF Metrics Extraction
                ...
                if target_pci is None and points:
                    next_pcis = [p['pci'] for p in points[matched_idx:min(len(points), matched_idx + 10)] if p['pci'] > 0 and p['pci'] != trigger_pt['pci']]
                ...
                rep_cnt = ep.get('rep_cnt') or a3_count_pre or (ep['events'][0].get('count') if ep.get('events') else 0)

                srv_pci = trigger_pt.get('pci', 0)
                srv_rsrp = trigger_pt.get('rsrp', -140.0)
                srv_sinr = trigger_pt.get('sinr', -20.0)
                start_rsrp = points[start_idx].get('rsrp', srv_rsrp) if points and start_idx < len(points) else srv_rsrp
                delta_rsrp = round(srv_rsrp - start_rsrp, 1)
                ...
                if is_volte_drop:
                    mute_sec = ep.get('mute_duration_sec') or ep.get('duration_sec') or 3.0
                    loss_pct = ep.get('loss_pct') or 100.0
                    base_badge = f"통화 묵음 및 RRE 거절 호 절단 ({mute_sec:.1f}초 묵음 / RTP Drop - Bye)"
                    base_title = f"통화 묵음 지속 및 RRE 거절 후 단말 강제 호 절단 (RTP Drop - Bye)"
                    root_cause_text = (
                        f"타겟 셀 신호 유입으로 인한 채널 간섭(SINR {srv_sinr:.1f} dB)으로 하향 링크 동기를 상실하여 RLF가 발생하였으며, 단말이 RRC Connection Reestablishment Request로 셀 복구를 시도했으나 기지국의 Reestablishment Reject로 연결이 거절됨.\n\n"
                        f"무선 베어러 단절 이후 하향 음성 RTP 패킷 수신이 완전히 중단되어 {mute_sec:.1f}초간 지속적인 통화 묵음(Audio Mute, 손실률 {loss_pct:.1f}%)이 발생함.\n\n"
                        f"단말 IMS 통화 엔진의 패킷 무수신 타이머가 만료됨에 따라 단말이 망으로 세션 종료 요청(SIP BYE Tx)을 송신하고 VoLTE 통화를 비정상 강제 절단함."
                    )
                    symptoms_text = (
                        f"• 전파 환경: 서빙 PCI {srv_pci} (RSRP {srv_rsrp:.1f} dBm, SINR {srv_sinr:.1f} dB)\n"
                        f"• 시그널링 경과: RLF 발생 ➔ Reestablishment Reject 회신 ➔ RTP 수신 차단({mute_sec:.1f}초) ➔ SIP BYE (Tx) 강제 송신\n"
                        f"• 사용자 체감: 통화 도중 {mute_sec:.1f}초간 상대방 음성이 전혀 들리지 않는 묵음 발생 후 통화 강제 끊김"
                    )

                elif is_rach_rlf:
                    base_badge = "상향 RACH 실패 RLF (Preamble 한계 초과)"
                    ...
                    root_cause_text = (
                        ...
                        f"수신 전계 변화({start_rsrp:.1f} dBm ➔ {srv_rsrp:.1f} dBm, Δ{delta_rsrp:+.1f} dB) 구간에서 상향 송신 실패가 결합되어 진행 중이던 세션이 강제 종료됨."
                    )
                    symptoms_text = (
                        f"• 전파 환경: 서빙 PCI {srv_pci} (RSRP {srv_rsrp:.1f} dBm, SINR {srv_sinr:.1f} dB) | {delta_rsrp:+.1f} dB 전계 변화\n"
                    ...
                    hop_cnt = ep.get('rep_cnt') or (len([c for c in full_pci_chain.split('➔') if c.strip()]) - 1) or 2
                    avg_delta = ep.get('delta_rsrp', 0.0)
                    hop_rate = round(hop_cnt / max(1.0, dur_sec), 1)
                    base_badge = f"다발 핑퐁 ({hop_cnt}회, {full_pci_chain})"
                    base_title = f"기지국 간 신호 중첩에 의한 다발 핑퐁 핸드오버 ({hop_cnt}회, {full_pci_chain})"
                    root_cause_text = (
                        f"복수 기지국({full_pci_chain}) 간 중첩 커버리지 구간에서 신호 세기 편차가 미미하고 핸드오버 히스테리시스/오프셋 마진이 부족하여, {dur_sec:.1f}초 동안 {hop_cnt}회에 걸쳐 왕복 핸드오버가 집중 발생함.\n\n"
                        f"짧은 시간 내 잦은 셀 변경으로 인해 기지국 및 단말의 C-Plane 시그널링 오버헤드가 급증하였으며, 핸드오버 중 무선 채널 불안정이 가중됨.\n\n"
                        f"평균 전계 편차(ΔRSRP {avg_delta:+.1f} dB) 내에서 불필요한 기지국 재선택이 반복되어 서비스 품질 저하 및 무선 링크 불안정을 유발함."
                    )
                    symptoms_text = (
                        f"• 전파 환경: 천이 기지국 그룹 ({full_pci_chain}) | 평균 천이 편차 ΔRSRP: {avg_delta:+.1f} dB\n"
                        f"• 시그널링 통계: {dur_sec:.1f}초간 총 {hop_cnt}회 연속 기지국 천이 (초당 약 {hop_rate}회 핸드오버)\n"
                        f"• 서비스 영향: 잦은 셀 스위칭으로 인한 순간 전송 지연 및 C-Plane 부하 가중"
                    )

                elif is_a3_drop or (is_rlf_reject and (rep_cnt >= 2 or target_pci)):
                    cnt_label = f" (A3 MR {rep_cnt}회)" if rep_cnt > 0 else ""
                    base_badge = f"타겟({tgt_str}) HO 방치 후 호 단절{cnt_label}"
                    base_title = f"타겟 기지국({tgt_str}) 핸드오버 방치 후 무선 링크 단절(RLF) 및 호 절단{cnt_label}"
                    rf_state_str = f"양호한 서빙 전계(RSRP {srv_rsrp:.1f} dBm)" if srv_rsrp >= -85.0 else f"서빙 전계(RSRP {srv_rsrp:.1f} dBm)"
                    root_cause_text = (
                        ...
                        f"타겟 셀 신호 유입으로 인한 채널 간섭(SINR {srv_sinr:.1f} dB)으로 하향 동기를 상실하여 무선 링크 단절(RadioLinkFailure)이 발생하였으며, 단말이 기지국 재수립(RRC Connection Reestablishment Request)을 시도했으나 대상 셀의 단말 컨텍스트 부재로 Reestablishment Reject를 수신하여 복구에 실패함.\n\n"
                        f"이로 인해 데이터 전송 베어러가 비정상 해제되며 진행 중이던 서비스 호가 완전히 단절됨."
                    )
                    symptoms_text = (
                        f"• 전파 환경: 서빙 PCI {srv_pci} (RSRP {srv_rsrp:.1f} dBm, SINR {srv_sinr:.1f} dB) | 타겟 후보 {tgt_str}\n"
                        f"• 시그널링 실패: A3 핸드오버 측정보고 {rep_cnt}회 전송 후 기지국 무응답 ➔ T310 만료 RLF ➔ Reestablishment Reject 수신\n"
                        f"• 서비스 영향: {dur_sec:.1f}초간 서비스 지연 후 무선 연결 강제 해제 (호 단절 발생)"
                    )
                    ...
                    root_cause_text = (
                        f"무선 채널 품질 저하(RSRP {srv_rsrp:.1f} dBm, SINR {srv_sinr:.1f} dB)로 하향 동기를 상실하여 무선 링크 단절(RadioLinkFailure)이 발생함.\n\n"
                        f"단말이 기지국 재수립(RRC Connection Reestablishment Request)을 시도했으나 기지국이 Reestablishment Reject를 회신하여 복구에 실패함.\n\n"
                        f"무선 링크 복구 실패로 인해 연결된 모든 베어러가 강제 해제되며 호 단절이 확정됨."
                    )
                    symptoms_text = (
                        f"• 전파 환경: 서빙 PCI {srv_pci} (RSRP {srv_rsrp:.1f} dBm, SINR {srv_sinr:.1f} dB)\n"
                    ...
                    root_cause_text = ep.get('root_cause') or f"단말이 타겟 셀({tgt_str})에 대해 A3 측정보고를 총 {rep_cnt}회 전송했으나 기지국에서 핸드오버 명령을 미발행하여 {dur_sec:.1f}초간 방치됨."
                    ...
                    root_cause_text = ep.get('root_cause') or f"기지국 간 전계 중첩 및 핸드오버 지연으로 인한 품질 저하 (서빙 PCI {srv_pci}, RSRP {srv_rsrp:.1f} dBm)"
                    symptoms_text = ep.get('symptoms') or f"구간 지속 시간: {dur_sec:.1f}초 | 무선 링크 품질 저하 발생"
        ...
        for ep in formatted_episodes:
            ...
            if time_diff <= 3.0:
                ...
                if should_merge:
                    ...
                    # Merge corridor
                    existing_coords = {f"{c[0]:.6f},{c[1]:.6f}" for c in prev_ep.get('corridor', [])}
    ...
    def generate_integrated_multi_port_map(
        ...
        def_lte_bw = def_port_data.get('lte_bw', 60.0)
