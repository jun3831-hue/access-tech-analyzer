    ...
    def extract_incident_episodes(
        ...
        if episodes:
            for ep_idx, ep in enumerate(episodes, 1):
                ...
                if has_call_drop:
                    if 'RTP Drop' in raw_title or 'RTP' in raw_title or 'VoLTE' in raw_title or 'DIAG_E_11' in diag_code:
                        candidates.append((1, "VoLTE 호 비정상 절단 (RTP Drop - Bye)", "통화 묵음(Audio Mute) 및 기지국 RRE 거절 후 강제 종료 (RTP Drop - Bye)"))
                    elif ep.get('has_unhandled_ho') or 'A3' in raw_title or '방치' in raw_title:
                ...
                # Evaluate Rank 3: Ping-Pong Handover / Multi-cell Loop / TAU Ping-Pong
                if 'TAU' in raw_title or 'DIAG_M_06' in diag_code:
                    candidates.append((3, "TAC 핑퐁 (빈번한 TAU)", "LTE TAC 경계 핑퐁 및 빈번한 TAU 발생"))
  ...
  function initLegend() {{
    ...
    if (currentMetric === 'pci' || currentMetric === 'nr_pci' || currentMetric === 'lte_pci') {{
      ...
      if (mapCriteria[targetCritKey]) {{
        const crit = mapCriteria[targetCritKey];
        const bwTag = (pdata && pdata.bw_str && (targetCritKey === 'pdcp_total' || targetCritKey === 'nr_mac' || targetCritKey === 'lte_mac')) 
          ? ` [${{pdata.bw_str}}]` 
          : '';
        metricTitle = crit.title + bwTag;
