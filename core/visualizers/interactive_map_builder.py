...
class InteractiveMapBuilder:
    ...
    def extract_trajectory_from_timeline(
        ...
        # Voice
        col_mos = self._find_col(df_timeline, ['MOS', '[Call & MOS P863(POLQA)]', 'Audio MOS', 'POLQA'])
        ...
        col_event = self._find_col(df_timeline, ['Event', 'Events', '[Call & Event LTE Events]', '[Call & Event 5G-NR/LTE Events]'])
        ...
        for idx, row in df_timeline.iterrows():
            ...
            points.append({
                ...
                "event": str(row[col_event]) if col_event and pd.notna(row[col_event]) else ""
            })
    ...
    def extract_incident_episodes(
        ...
        for p in points:
            ...
            if p.get('event') and p['event'] not in sig_list:
                sig_list.append(p['event'])
            if sig_list:
        ...
        if episodes:
            for ep_idx, ep in enumerate(episodes, 1):
                ...
                if is_volte_drop:
                    ...
                    is_clean_rf = is_clean_rf_condition(srv_rsrp, srv_sinr) or (srv_sinr is not None and srv_sinr >= 10.0 and not is_weak_coverage_condition(srv_rsrp))

                    # Fact-based verification of Reestablishment Reject existence
                    has_rre_reject = ep.get('has_reject', False) or any(
                        'reestablishmentreject' in str(p.get('l3_sig', '')).lower() or 'reject' in str(p.get('l3_sig', '')).lower()
                        for p in points[start_idx:end_idx + 1]
                    )
                    if has_rre_reject:
                        rre_recovery_clause = "단말이 기지국 재수립(RRC Connection Reestablishment Request)을 시도했으나 대상 셀의 단말 컨텍스트 부재로 Reestablishment Reject를 수신하여 복구에 실패함."
                        sig_fail_rre = "Reestablishment Reject 수신"
                    else:
                        rre_recovery_clause = "단말의 무선 링크 재수립(RRC Connection Reestablishment) 시도에도 기지국의 유효 응답 부재로 연결 복구에 도달하지 못함."
                        sig_fail_rre = "RRE 응답 부재 (복구 실패)"
                    ...
                    if is_clean_rf:
                        root_cause_text = (
                            ...
                            f"기지국의 핸드오버 명령 미발행 상태가 지속되며 무선 링크 단절(RadioLinkFailure)이 발생하였으며, {rre_recovery_clause} "
                            f"이로 인해 데이터 전송 베어러가 비정상 해제되며 진행 중이던 서비스 호가 완전히 단절됨."
                        )
                    else:
                        if drop_tag_str:
                            rf_fail_cause = f"{drop_tag_str} 상태에서"
                        elif is_pilot_pollution_condition(srv_rsrp, srv_sinr):
                            rf_fail_cause = f"타겟 셀 신호 유입으로 인한 채널 간섭(SINR {srv_sinr:.1f} dB) 상태에서"
                        elif is_weak_coverage_condition(srv_rsrp):
                            rf_fail_cause = f"서빙 기지국 음영 진입(RSRP {srv_rsrp:.1f} dBm) 상태에서"
                        else:
                            rf_fail_cause = f"서빙 채널 상태(SINR {srv_sinr:.1f} dB)에서" if srv_sinr is not None else "하향 무선 채널 불안정 상태에서"

                        root_cause_text = (
                            f"단말이 {rf_state_str} 환경에서 타겟 기지국({tgt_str})으로의 핸드오버를 위해 A3 측정보고(MeasurementReport)를 총 {rep_cnt}회 전송했으나, 기지국에서 핸드오버 명령(RRCConnectionReconfiguration)을 발행하지 않고 방치함. "
                            f"{rf_fail_cause} 하향 물리계층 동기 타이머(T310) 만료로 무선 링크 단절(RadioLinkFailure)이 발생하였으며, {rre_recovery_clause} "
                            f"이로 인해 데이터 전송 베어러가 비정상 해제되며 진행 중이던 서비스 호가 완전히 단절됨."
                        )
                    dur_clause = f"{dur_sec:.1f}초간 " if dur_sec is not None else ""
                    symptoms_text = (
                        f"• 전파 환경: {rf_summary_str} | 타겟 후보 {tgt_str}\n"
                        f"• 시그널링 실패: A3 핸드오버 측정보고 {rep_cnt}회 전송 후 기지국 무응답 ➔ T310 만료 RLF ➔ {sig_fail_rre}\n"
                    ...
                    if std_cause == 'handoverFailure':
                        ...
                        base_title = f"타겟 기지국({tgt_str}) 핸드오버 실패(T304 만료) 후 호 절단"
                        ...
                        base_badge = "RLF 및 복구 실패 호 단절"
                        base_title = "기지국 무선 링크 붕괴(RLF) 및 복구 실패 호 단절"
                        cause_detail_str = "무선 링크 단절(RLF)"

                    if has_rre_reject:
                        base_title = base_title.replace("호 절단", "RRE 거절 호 절단").replace("호 단절", "RRE 거절 호 단절")
                        rre_recovery_clause = "단말이 기지국 재수립(RRC Connection Reestablishment Request)을 시도했으나 기지국이 Reestablishment Reject를 회신하여 복구에 실패함."
                        sig_fail_chain = "RLF 발생 ➔ Reestablishment Request ➔ Reestablishment Reject 수신"
                    else:
                        rre_recovery_clause = "무선 링크 복구 시도에도 기지국의 유효 응답 부재로 RRC 재수립에 도달하지 못함."
                        sig_fail_chain = "RLF 발생 ➔ RRE 시도 후 기지국 무응답 ➔ RRC 복구 실패"
                ...
                # 2. Parse fine-grained timeline steps from story_steps / timeline
                for step in (ep.get('story_steps') or ep.get('timeline') or []):
                    ...
                    if m_step:
                        ...
                        if "BYE" in step_text:
                            ...
                            note_map[step_t] = step_text
                        elif "eventA3" in step_text or "A3 MR" in step_text or "측정보고" in step_text:
                            event_map[step_t] = "⚠️ eventA3 MeasurementReport"
                            note_map[step_t] = step_text
                        elif "RLF" in step_text or "RadioLinkFailure" in step_text or "무선 링크 단절" in step_text:
                ...
                for cur_time in sorted_timeline_times:
                    ...
                    # Fallback to points l3_sig if not explicitly in event_map
                    if not sig_msg or sig_msg == "-":
                        pt_sig = l3_sig_by_sec.get(cur_time, "-")
                        if pt_sig and pt_sig != "-":
                            sig_msg = pt_sig

                    # Specialize A3 unhandled progression (T+1s ~ T+end)
                    is_a3_episode = (is_a3_drop or ('A3' in raw_title) or ('방치' in raw_title))
                    if is_a3_episode and rel_sec > 0 and (end_sec < 0 or cur_sec <= end_sec):
                        if sig_msg == "MeasurementReport" or (sig_msg and "MeasurementReport" in sig_msg and "eventA3" not in sig_msg):
                            sig_msg = "eventA3 MeasurementReport"
                        if not note or note == "-" or note == "MeasurementReport" or "MeasurementReport" in note:
                            note = "A3 측정보고 지속 송신 (기지국 HO 명령 미발행)"

                    # Prevent redundant note if identical to sig_msg
                    if note and sig_msg and note.strip() == sig_msg.strip():
                        note = "-"
    ...
    def generate_integrated_multi_port_map(
        ...
        for port_key, pdata in port_data_dict.items():
            ...
            if scenario == "Voice":
                fourth_btn_name = ("🎙️ VoLTE MOS" if network_mode != "SA" else "🎙️ VoNR MOS") if has_valid_mos else "🎙️ MOS (미측정)"
                fourth_btn_key = "mos"
...
<script>
  ...
  function getMechanismAnalysisHtml(ep) {{
    const title = (ep.title || '').toLowerCase();
    const cause = (ep.root_cause || '').toLowerCase();
    const combined = title + ' ' + cause;
    
    // 시그널링 메시지 모음
    const sigMsgs = (ep.timeline || []).map(r => (r.sig_msg || '') + ' ' + (r.note || '')).join(' ').toLowerCase();
    const fullText = combined + ' ' + sigMsgs;

    let items = [];

    // 1. RLF 원인 4종 판별
    let isT310 = fullText.includes('t310') || fullText.includes('t310-expiry') || fullText.includes('동기 상실') || fullText.includes('out-of-sync');
    let isRach = fullText.includes('randomaccessproblem') || fullText.includes('rach 실패') || fullText.includes('프리앰블') || fullText.includes('preamble');
    let isRlc = fullText.includes('rlc-maxnumretx') || fullText.includes('rlc 재전송') || fullText.includes('maxretx');
    let isReconfig = fullText.includes('reconfigurationfailure') || fullText.includes('재구성 실패') || fullText.includes('재구성 거절');

    if (isT310) {{
      items.push({{
        badge: '🚨 3GPP RLF 원인: t310-Expiry (하향 물리 링크 단절)',
        color: '#ef4444',
        desc: '<b>발생 상황:</b> 급격한 음영지역 진입(터널, 지하차도, 건물 차폐) 또는 동일 주파수 인접 셀의 극심한 간섭으로 인해 단말이 기지국 하향 PDCCH 제어채널을 전혀 복호화하지 못하고 BLER(블록 에러율)이 100%에 달해 Qout(비동기 기준) 조건을 20회(N310) 연속 감지했을 때 T310이 구동되며, 타이머 만료 시점까지 동기 복구(N311회 In-sync) 또는 핸드오버 완료 메시지를 수신하지 못해 무선 링크가 완전 단절된 상황입니다.'
      }});
    }}
    if (isRach) {{
      items.push({{
        badge: '🚨 3GPP RLF 원인: randomAccessProblem (PRACH 프리앰블 실패)',
        color: '#ef4444',
        desc: '<b>발생 상황:</b> 하향 전계는 감지되나 단말의 상향 전송 출력(Tx Power) 한계로 기지국 안테나까지 신호가 도달하지 못하는 링크 불균형(Link Imbalance) 환경, 또는 심각한 상향 잡음/타 단말 프리앰블 충돌로 인해 기지국이 프리앰블을 검출하지 못하여 최대 허용 전송 횟수(preambleTransMax, 통상 10~20회) 동안 RAR(응답)을 한 번도 받지 못해 발생한 상황입니다.'
      }});
    }}
    if (isRlc) {{
      items.push({{
        badge: '🚨 3GPP RLF 원인: rlc-MaxNumRetx (상향 전송로 마비)',
        color: '#ef4444',
        desc: '<b>발생 상황:</b> 무선 채널의 깊은 페이딩(Deep Fading) 또는 심한 상향 간섭으로 RLC AM 모드 패킷 전송에 대해 NACK이 연속 발생하거나 수신 확인(Status Report)이 두절되어, 최대 허용 재전송 임계치(maxRetxThreshold, 통상 8~16회)를 소진할 때까지 패킷을 정상 전달하지 못하고 상향 링크가 완전히 마비된 상황입니다.'
      }});
    }}
    if (isReconfig) {{
      items.push({{
        badge: '🚨 3GPP RLF 원인: reconfigurationFailure (RRC 재구성 적용 불가)',
        color: '#ef4444',
        desc: '<b>발생 상황:</b> 기지국이 전달한 무선 베어러 또는 무선 자원 설정 파라미터가 단말의 하드웨어/프로토콜 Capability 범위를 초과하거나 설정 충돌이 발생하여, 단말이 RRCConnectionReconfiguration 구성을 적용하지 못하고 거부한 상황입니다.'
      }});
    }}

    // 2. 핵심 타이머 3종 (RLF 또는 복구 절차가 수반된 경우)
    let hasRlfOrRre = fullText.includes('rlf') || fullText.includes('t310') || fullText.includes('reestablishment') || fullText.includes('재수립') || fullText.includes('호 단절') || fullText.includes('호 절단');
    if (hasRlfOrRre) {{
      items.push({{
        badge: '⏱️ 관련 3GPP 핵심 타이머 규격 (T310 / T311 / T301)',
        color: '#38bdf8',
        desc: '• <b>T310 타이머 (RLF 판정):</b> 하향 PDCCH 비동기(N310회 Qout) 감지 시 기동되어 무선 링크 복구(N311 Qin) 또는 핸드오버 완료를 대기함. 만료 시 RLF를 선언하고 즉시 T311 타이머를 구동함.<br>' +
              '• <b>T311 타이머 (셀 재선택 대기):</b> RLF 선언 직후 단말이 통화 유지를 위해 복구 가능한 적합 셀(Suitable Cell)을 탐색/재선택하는 시간. 만료 시 셀 복구 실패로 RRC_IDLE로 천이(호 절단 확정).<br>' +
              '• <b>T301 타이머 (RRE 응답 대기):</b> 재선택한 기지국으로 RRCConnectionReestablishmentRequest 전송 후 기지국 응답을 대기하는 시간. 만료 시 재수립 실패로 호 절단됨.'
      }});
    }}

    // 3. 이동성(HO) 결함 2종
    let isTooEarly = fullText.includes('too early') || fullText.includes('조기 핸드오버') || (fullText.includes('핸드오버 직후') && fullText.includes('rlf'));
    let isTooLate = fullText.includes('too late') || fullText.includes('지연 핸드오버') || fullText.includes('ho 방치') || fullText.includes('요청 무응답') || fullText.includes('a3 mr');
    
    if (isTooEarly) {{
      items.push({{
        badge: '🔄 3GPP 핸드오버 결함: Too Early Handover (조기 핸드오버)',
        color: '#f59e0b',
        desc: '<b>발생 상황:</b> 단말이 타겟 셀로 핸드오버를 진입한 직후 짧은 시간 내에 RLF가 발생하여, 이전 서빙 셀로 다시 재수립(RRE)을 시도하는 결함입니다. 타겟 셀의 커버리지가 불충분하거나 핸드오버 파라미터(Time To Trigger, A3 Offset)가 너무 성급하게 설정되었을 때 발생합니다.'
      }});
    }}
    if (isTooLate) {{
      items.push({{
        badge: '🔄 3GPP 핸드오버 결함: Too Late Handover / 방치 (지연 핸드오버)',
        color: '#f59e0b',
        desc: '<b>발생 상황:</b> 서빙 셀 신호가 급격히 감쇠하는 동안 핸드오버를 적기에 완료하지 못하고 서빙 셀에서 먼저 RLF가 발생한 후 우세한 인접 셀로 재수립(RRE)을 시도하거나, 단말이 A3 측정보고를 지속 송신했음에도 기지국이 Reconfig 명령을 내리지 않아 서빙 전계 고갈로 단절에 이른 상황입니다.'
      }});
    }}

    // 4. 5G NSA SCG 결함 4종
    let isScg = fullText.includes('scg') || fullText.includes('en-dc') || fullText.includes('5g 단절') || fullText.includes('nr');
    if (isScg && (fullText.includes('fail') || fullText.includes('단절') || fullText.includes('해제') || fullText.includes('drop'))) {{
      let scgDetails = [];
      if (fullText.includes('t304') || fullText.includes('synchreconfig')) {{
        scgDetails.push('• <b>synchReconfigFailureSCG (T304-SCG 만료):</b> LTE 기지국 지시로 5G PSCell 추가/변경을 시도했으나 제한 시간(T304) 내 5G RACH 및 동기 획득 실패');
      }}
      if (fullText.includes('scg-t310') || fullText.includes('5g rlf')) {{
        scgDetails.push('• <b>scg-t310-Expiry (5G 빔 고갈):</b> 5G gNodeB 하향 빔 전계 급감으로 SCG T310 만료 시까지 5G 신호 복구 실패');
      }}
      if (fullText.includes('rach') || fullText.includes('preamble')) {{
        scgDetails.push('• <b>scg-randomAccessProblem:</b> 5G 빔 실패 복구(BFR) 또는 상향 자원 요청 RACH에서 프리앰블 최대 전송 초과 실패');
      }}
      if (fullText.includes('rlc') || fullText.includes('retx')) {{
        scgDetails.push('• <b>scg-rlc-MaxNumRetx:</b> 5G NR 전용 데이터 베어러(DRB) 상향 패킷 최대 재전송 횟수 초과');
      }}
      if (scgDetails.length === 0) {{
        scgDetails.push('• <b>synchReconfigFailureSCG:</b> 5G 셀 동기 획득 실패 | • <b>scg-t310-Expiry:</b> 5G 빔 전계 단절');
      }}
      items.push({{
        badge: '⚡ 3GPP 5G NSA SCG 결함 분석 (SCGFailureInformation-NR)',
        color: '#a855f7',
        desc: '<b>발생 상황:</b> 5G NSA 이중연결 상태에서 5G PSCell 링크가 중단된 세부 원인:<br>' + scgDetails.join('<br>')
      }});
    }}
    ...
    items.forEach(it => {{
      cardsHtml += `
        <div style="background-color: #0b1329; border: 1px solid #334155; border-left: 4px solid ${{it.color}}; padding: 10px 14px; border-radius: 6px; margin-top: 8px;">
          <div style="font-weight: 700; color: ${{it.color}}; font-size: 11.5px; margin-bottom: 4px;">
            ${{it.badge}}
          </div>
          <div style="font-size: 11px; color: #e2e8f0; line-height: 1.55;">
            ${{it.desc}}
          </div>
        </div>
      `;
