                ...
                if is_volte_drop:
                    ...
                    if is_clean_rf:
                        root_cause_text = (
                            ...
                            f"기지국의 핸드오버 명령 미발행 상태가 지속되며 무선 링크 단절(RadioLinkFailure)이 발생하였으며, 단말이 기지국 재수립(RRC Connection Reestablishment Request)을 시도했으나 대상 셀의 단말 컨텍스트 부재로 Reestablishment Reject를 수신하여 복구에 실패함. "
                        ...
                        root_cause_text = (
                            f"단말이 {rf_state_str} 환경에서 타겟 기지국({tgt_str})으로의 핸드오버를 위해 A3 측정보고(MeasurementReport)를 총 {rep_cnt}회 전송했으나, 기지국에서 핸드오버 명령(RRCConnectionReconfiguration)을 발행하지 않고 방치함. "
                            f"{rf_fail_cause} 하향 물리계층 동기 타이머(T310) 만료로 무선 링크 단절(RadioLinkFailure)이 발생하였으며, 단말이 기지국 재수립(RRC Connection Reestablishment Request)을 시도했으나 대상 셀의 단말 컨텍스트 부재로 Reestablishment Reject를 수신하여 복구에 실패함. "
                            f"이로 인해 데이터 전송 베어러가 비정상 해제되며 진행 중이던 서비스 호가 완전히 단절됨."
                        )
