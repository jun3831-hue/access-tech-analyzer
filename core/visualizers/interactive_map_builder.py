        ...
        if episodes:
            for ep_idx, ep in enumerate(episodes, 1):
                ...
                if is_volte_drop:
                    ...
                    sinr_desc = f"채널 간섭(SINR {srv_sinr:.1f} dB)" if srv_sinr is not None else "채널 간섭"
                    ...
                    root_cause_text = (
                        f"타겟 셀 신호 유입으로 인한 {sinr_desc}으로 하향 링크 동기를 상실하여 RLF가 발생하였으며, 단말이 RRC Connection Reestablishment Request로 셀 복구를 시도했으나 기지국의 Reestablishment Reject로 연결이 거절됨.\n\n"
                    ...
                    rf_change_desc = f"수신 전계 변화({start_rsrp:.1f} dBm ➔ {srv_rsrp:.1f} dBm, Δ{delta_rsrp:+.1f} dB) 구간에서 " if (start_rsrp is not None and srv_rsrp is not None and delta_rsrp is not None) else ""
                    root_cause_text = (
                        ...
                        f"{rf_change_desc}상향 송신 실패가 결합되어 진행 중이던 세션이 강제 종료됨."
                    )
                    delta_tag = f" | {delta_rsrp:+.1f} dB 전계 변화" if delta_rsrp is not None else ""
                    symptoms_text = (
                    ...
                    root_cause_text = (
                        ...
                        f"타겟 셀 신호 유입으로 인한 {sinr_desc}으로 하향 동기를 상실하여 무선 링크 단절(RadioLinkFailure)이 발생하였으며, 단말이 기지국 재수립(RRC Connection Reestablishment Request)을 시도했으나 대상 셀의 단말 컨텍스트 부재로 Reestablishment Reject를 수신하여 복구에 실패함.\n\n"
                    ...
                    rf_metric_str = f"RSRP {srv_rsrp:.1f} dBm, SINR {srv_sinr:.1f} dB" if (srv_rsrp is not None and srv_sinr is not None) else "무선 채널 품질 저하"
                    root_cause_text = (
                        f"무선 채널 품질 저하({rf_metric_str})로 하향 동기를 상실하여 무선 링크 단절(RadioLinkFailure)이 발생함.\n\n"
